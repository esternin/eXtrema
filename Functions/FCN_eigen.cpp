...
FCN_eigen FCN_eigen::fcn_eigen_;
...
void FCN_eigen::ProcessArgument( bool firstArgument, int ndmEff, int &nLoop )
{
  ...
  {
    if( ndmEff != 2 )
      throw EExpressionError("argument of EIGEN function must be a matrix");
...
void FCN_eigen::ArrayEval( Workspace *ws )
{
  ...
  else    // argument 1 must be a workspace
  {
    ...
    if( w1->IsEmpty() )
      throw EExpressionError( "EIGEN: argument workspace is empty" );
    d.assign( w1->GetData().begin(), w1->GetData().end() );
    rowSize = w1->GetDimMag(0);
    colSize = w1->GetDimMag(1);
  }
  if( rowSize != colSize )
    throw EExpressionError( "EIGEN: argument must be a square matrix" );
  ...
  for( std::size_t j=1; j<=n; ++j )
  {
    for( std::size_t i=1; i<=n; ++i )
    {
      if( d[i-1+(j-1)*n] != d[j-1+(i-1)*n] )
        throw EExpressionError( "EIGEN: argument must be a symmetric matrix" );
    }
  }
