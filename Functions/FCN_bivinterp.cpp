...
FCN_bivariateInterpolate FCN_bivariateInterpolate::fcn_bivariateInterpolate_;
...
void FCN_bivariateInterpolate::ProcessArgument( bool firstArgument, int ndmEff, int &nLoop )
{
  ...
  {
    if( ndmEff != 1 )
      throw EExpressionError("BIVINTERP: first argument must be a vector");
  ...
  else
  {
    ...
    switch ( ++numberOfArguments_ )
    {
      case 2:
        if( ndmEff != 1 )
          throw EExpressionError("BIVINTERP: second argument must be a vector");
        break;
      case 3:
        if( ndmEff != 2 )
          throw EExpressionError("BIVINTERP: third argument must be a matrix");
        break;
      case 4:
        if( ndmEff != 1 )
          throw EExpressionError("BIVINTERP: fourth argument must be a vector");
        break;
      case 5:
        if( ndmEff != 1 )
          throw EExpressionError("BIVINTERP: fifth argument must be a vector");
        break;
    }
  }
}
...
void FCN_bivariateInterpolate::ArrayEval( Workspace *ws )
{
  ...
  else
  {
    ...
    if( w->IsEmpty() )
      throw EExpressionError( "BIVINTERP: first argument workspace is empty" );
    d1.assign( w->GetData().begin(), w->GetData().end() );
  }
  int size1 = d1.size();
  if( size1 < 2 )
    throw EExpressionError("BIVINTERP: independent vector length < 2");
  //
  ...
  else
  {
    ...
    if( w->IsEmpty() )
      throw EExpressionError( "BIVINTERP: second argument workspace is empty" );
    d2.assign( w->GetData().begin(), w->GetData().end() );
  }
  int size2 = d2.size();
  if( size2 < 2 )
    throw EExpressionError("BIVINTERP: dependent vector length < 2");
  //
  int size3a[2];
  ...
  else
  {
    ...
    if( w->IsEmpty() )
      throw EExpressionError( "BIVINTERP: input matrix argument workspace is empty" );
    d3.assign( w->GetData().begin(), w->GetData().end() );
    size3a[0] = w->GetDimMag(0);
    size3a[1] = w->GetDimMag(1);
  }
  if( size3a[0] != size2 )
    throw EExpressionError("BIVINTERP: #rows of input matrix must = dependent vector length");
  if( size3a[1] != size1 )
    throw EExpressionError("BIVINTERP: #columns of input matrix must = independent vector length");
  //
  ...
  else
  {
    ...
    if( w->IsEmpty() )
      throw EExpressionError( "BIVINTERP: fourth argument workspace is empty" );
    d4.assign( w->GetData().begin(), w->GetData().end() );
  }
  int size4 = d4.size();
  if( size4 < 1 )
    throw EExpressionError("BIVINTERP: output independent vector is empty");
  //
  ...
  else
  {
    ...
    if( w->IsEmpty() )
      throw EExpressionError( "BIVINTERP: fifth argument workspace is empty" );
    d5.assign( w->GetData().begin(), w->GetData().end() );
  }
  int size5 = d5.size();
  if( size5 != size4 )
    throw EExpressionError("BIVINTERP: output dependent vector length must = independent vector length");
  //
  ...
  {
    throw EExpressionError( ExString("BIVINTERP: ")+e.what() );
  }
  //
