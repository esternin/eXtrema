...
FCN_volume FCN_volume::fcn_volume_;
...
void FCN_volume::ProcessArgument( bool firstArgument, int ndmEff, int &nLoop )
{
  ...
  {
    if( ndmEff != 1 )
      throw EExpressionError("VOLUME: first argument must be a vector");
  ...
  else
  {
    ...
    {
      if( ndmEff != 1 )
        throw EExpressionError("VOLUME: second argument must be a vector");
    }
    else if( numberOfArguments_ == 3 )
    {
      if( ndmEff!=1 && ndmEff!=2 )
        throw EExpressionError("VOLUME: third argument must be a vector or a matrix");
    }
  }
}
...
void FCN_volume::ArrayEval( Workspace *ws )
{
  ...
  else
  {
    ...
    if( w1->IsEmpty() )
      throw EExpressionError( "VOLUME: first argument workspace is empty" );
  ...
  else
  {
    ...
    if( w2->IsEmpty() )
      throw EExpressionError( "VOLUME: second argument workspace is empty" );
    y.assign( w2->GetData().begin(), w2->GetData().end() );
  }
  std::size_t size2 = y.size();
  if( size2 != size1 )throw EExpressionError("VOLUME: vectors have different lengths");
  //
  bool matrix = false;
  ...
  else
  {
    ...
    if( w3->IsEmpty() )
      throw EExpressionError( "VOLUME: third argument workspace is empty" );
  ...
  if( matrix )
  {
    if( nrows != size2 )throw EExpressionError(
      "VOLUME: length of dependent vector not equal to number of rows of the matrix");
    if( ncols != size1 )throw EExpressionError(
      "VOLUME: length of independent vector not equal to number of columns of the matrix");
  ...
  {
    if( size3 != size1 )throw EExpressionError("VOLUME: vectors have different lengths");
    if( size1 < 4 )throw EExpressionError("VOLUME: vector length < 4");
    try
    {
