...
void FCN_det::ProcessArgument( bool firstArgument, int ndmEff, int &nLoop )
{
  ...
  {
    if( ndmEff != 2 )
      throw EExpressionError("DET: argument must be a matrix");
...
void FCN_det::ArrayEval( Workspace *ws )
{
  ...
  else
  {
    ...
    if( w1->IsEmpty() )
      throw EExpressionError( "DET: first argument workspace is empty" );
  ...
  if( size1a[0] != size1a[1] )
    throw EExpressionError("DET: first argument must be a square matrix");
  int size1 = size1a[0];
  //
  if( size1 < 2 )
    throw EExpressionError("DET: matrix dimension < 2");
  //
