...
FCN_integral FCN_integral::fcn_integral_;
...
void FCN_integral::ProcessArgument( bool firstArgument, int ndmEff, int &nLoop )
{
  ...
  {
    if( ndmEff != 1 )
      throw EExpressionError("INTEGRAL: first argument must be a vector");
  ...
  else
  {
    ...
    if( numberOfArguments_==2 && ndmEff!=1 )
      throw EExpressionError("INTEGRAL: second argument must be a vector");
  }
}
...
void FCN_integral::ArrayEval( Workspace *ws )
{
  ...
  else
  {
    ...
    if( w1->IsEmpty() )
      throw EExpressionError( "INTEGRAL: first argument workspace is empty" );
  ...
  else
  {
    ...
    if( w2->IsEmpty() )throw EExpressionError( "INTEGRAL: second argument workspace is empty" );
    d2.assign( w2->GetData().begin(), w2->GetData().end() );
  }
  std::size_t size2 = d2.size();
  //
  if( size2 != size1 )
    throw EExpressionError("INTEGRAL: vectors have different lengths");
  if( size1 < 2 )
    throw EExpressionError("INTEGRAL: independent vector length < 2");
  //
  ...
  else
  {
    if( arg3->IsaWorkspace() )
      ...
      throw EExpressionError("INTEGRAL: third argument must be character");
    s3.ToUpper();
  }
  for( std::size_t i=1; i<size1; ++i )
  {
    if( d1[i] <= d1[i-1] )throw EExpressionError(
      "INTEGRAL: independent vector must be strictly monotonically increasing" );
  }
  ...
  else throw EExpressionError( "INTEGRAL: invalid third parameter" );
  //
