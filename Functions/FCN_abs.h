...
class FUNCTIONS_LINKAGE_ FCN_abs : public ScalarFunction
{
public:
  static FCN_abs *Definition()
  { return &fcn_abs_; }
  
  void ScalarEval( int j, std::vector<double> &rStack ) const
  { rStack[j] = fabs( rStack[j] ); }
  
private:
  FCN_abs() : ScalarFunction( "ABS" )
  {}

  static FCN_abs fcn_abs_;
};
