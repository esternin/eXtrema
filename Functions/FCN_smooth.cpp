...
FCN_smooth FCN_smooth::fcn_smooth_;
...
void FCN_smooth::ProcessArgument( bool firstArgument, int ndmEff, int &nLoop )
{
  ...
  {
    if( ndmEff != 1 )
      throw EExpressionError("SMOOTH: first argument must be a vector");
  ...
  else
  {
    ...
    switch (++numberOfArguments_)
    {
      case 2:
        if( ndmEff != 1 )
          throw EExpressionError("SMOOTH: second argument must be a vector");
        break;
      case 3:
        if( ndmEff != 1 )
          throw EExpressionError("SMOOTH: third argument must be a vector");
        break;
      case 4:
        if( ndmEff != 1 )
          throw EExpressionError("SMOOTH: fourth argument must be a vector");
        break;
    }
  }
}
...
void FCN_smooth::ArrayEval( Workspace *ws )
{
  ...
  else
  {
    ...
    if( w1->IsEmpty() )
      throw EExpressionError( "SMOOTH: first argument workspace is empty" );
  ...
  else
  {
    ...
    if( w2->IsEmpty() )
      throw EExpressionError( "SMOOTH: second argument workspace is empty" );
    d2.assign( w2->GetData().begin(), w2->GetData().end() );
  }
  std::size_t size2 = d2.size();
  //
  if( size2 != size1 )
    throw EExpressionError("SMOOTH: vectors have different lengths");
  if( size1 < 2 )
    throw EExpressionError("SMOOTH: independent vector length < 2");
  //
  ...
  else
  {
    ...
    if( w3->IsEmpty() )
      throw EExpressionError( "SMOOTH: third argument workspace is empty" );
  ...
  {
    if( d1[i] <= d1[i-1] )
      throw EExpressionError("SMOOTH: independent vector not monotonically increasing");
  }
  for( std::size_t i=1; i<size3; ++i )
  {
    if( d3[i] <= d3[i-1] )
      throw EExpressionError("SMOOTH: output independent vector not monotonically increasing");
  }
  if( d3[0]<d1[0] || d3[size3-1]>d1[size1-1] )
    throw EExpressionError("SMOOTH: output independent vector has values outside range of input independent vector");
  //
  ...
  if( numberOfArguments_ == 4 ) // arg4 is the weights
  {
    ...
    else
    {
      ...
      if( w4->IsEmpty() )
        throw EExpressionError( "SMOOTH: fourth argument workspace is empty" );
