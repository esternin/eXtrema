...
FCN_index FCN_index::fcn_index_;
...
void FCN_index::ArrayEval( Workspace *ws )
{
  ...
  if( !arg1->IsaTString() )
    throw EExpressionError( "INDEX: first argument must be character" );
  if( !arg2->IsaTString() )
    throw EExpressionError( "INDEX: second argument must be character" );
