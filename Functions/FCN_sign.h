...
class FUNCTIONS_LINKAGE_ FCN_sign : public ScalarFunction
{
public:
  static FCN_sign *Definition()
  { return &fcn_sign_; }
  
  void ScalarEval( int j, std::vector<double> &rStack ) const
  {
    double d = 1.0;
    if( rStack[j+1] < 0.0 )d = -1;
    rStack[j] = fabs(rStack[j])*d;
    rStack.pop_back();
  }

private:
  FCN_sign() : ScalarFunction( "SIGN", 2, 2 )
  {}
