...
void FCN_join::ProcessArgument( bool firstArgument, int ndmEff, int &nLoop )
{
  ...
  {
    if( ndmEff != 1 )
      throw EExpressionError("JOIN: first argument must be a vector");
  ...
  else
  {
    ...
    if( numberOfArguments_==2 && ndmEff!=1 )
      throw EExpressionError("JOIN: second argument must be a vector");
  }
}
...
void FCN_join::ArrayEval( Workspace *ws )
{
  ...
  else
  {
    ...
    if( w1->IsEmpty() )throw EExpressionError( "JOIN: first argument workspace is empty" );
  ...
  else
  {
    ...
    if( w2->IsEmpty() )
      throw EExpressionError( "JOIN: second argument workspace is empty" );
