...
class FUNCTIONS_LINKAGE_ FCN_ln : public ScalarFunction
{
public:
  static FCN_ln *Definition()
  { return &fcn_ln_; }
  
  void ScalarEval( int j, std::vector<double> &rStack ) const
  {
    if( rStack[j] <= 0.0 )throw EExpressionError("LN: argument <= 0");
    rStack[j] = log( rStack[j] );
  }

private:
  FCN_ln() : ScalarFunction( "LN" )
  {}
  
  static FCN_ln fcn_ln_;
};
