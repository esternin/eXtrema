...
FCN_savgol FCN_savgol::fcn_savgol_;
...
void FCN_savgol::ProcessArgument( bool firstArgument, int ndmEff, int &nLoop )
{
  ...
  {
    if( ndmEff != 0 )
      throw EExpressionError("SAVGOL: first argument must be a scalar");
  ...
  else
  {
    ...
    {
      if( ndmEff != 0 )
        throw EExpressionError("SAVGOL: second argument must be a scalar");
      numberOfArguments_ = 2;
    }
    else if( numberOfArguments_ == 2 )
    {
      if( ndmEff != 1 )
        throw EExpressionError("SAVGOL: third argument must be a vector");
...
void FCN_savgol::ArrayEval( Workspace *ws )
{
  ...
  if( filterOrder!=2 && filterOrder!=4 )
    throw EExpressionError("SAVGOL: first argument must be 2 or 4");
  //
  ...
  else
  {
    ...
    if( w->IsEmpty() )
      throw EExpressionError( "SAVGOL: third argument workspace is empty" );
    yIn.assign( w->GetData().begin(), w->GetData().end() );
    size = w->GetDimMag(0);
  }
  if( size < 2 )throw EExpressionError("SAVGOL: third argument length must be >= 2");
  //
