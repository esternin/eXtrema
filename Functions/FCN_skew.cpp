...
void FCN_skew::ArrayEval( Workspace *ws )
{
  ...
  if( arg1->IsaNVariable() )
  {
    ...
    if( ndm1 != 1 )
    {
      if( numberOfArguments_ == 1 )
        throw EExpressionError( "SKEW: argument must be a vector" );
      else
        throw EExpressionError( "SKEW: argument 1 must be a vector" );
    }
  ...
  else if( arg1->IsaWorkspace() )
  {
    ...
    {
      if( numberOfArguments_ == 1 )
        throw EExpressionError( "SKEW: argument workspace is empty" );
      else
        throw EExpressionError( "SKEW: argument 1 workspace is empty" );
    }
  ...
  if( numberOfArguments_ == 2 )
  {
    ...
    if( arg2->IsaNVariable() )
    {
      ...
      if( ndm2 != 1 )
        throw EExpressionError( "SKEW: argument 2 must be a vector" );
    ...
    else if( arg2->IsaWorkspace() )
    {
      ...
      if( w->IsEmpty() )
        throw EExpressionError( "SKEW: argument 2 workspace is empty" );
  ...
  if( numberOfArguments_ == 2 )
  {
    ...
    //
    if( size1 != size2 )
      throw EExpressionError( "SKEW: arguments must have the same length" );
    ...
    if( wsum == 0.0 )
      throw EExpressionError( "SKEW: sum of the weights = 0" );
  ...
  if( var < 0.0 )throw EExpressionError( "SKEW: the variance is negative" );
  if( var == 0.0 )throw EExpressionError( "SKEW: the variance is zero" );
