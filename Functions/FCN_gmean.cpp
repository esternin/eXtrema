...
FCN_gmean FCN_gmean::fcn_gmean_;
...
void FCN_gmean::ArrayEval( Workspace *ws )
{
  ...
  if( arg1->IsaNVariable() )
  {
    ...
    if( ndm1 != 1 )
    {
      if( numberOfArguments_ == 1 )
        throw EExpressionError( "GMEAN: argument must be a vector" );
      else
        throw EExpressionError( "GMEAN: argument 1 must be a vector" );
    }
  ...
  else if( arg1->IsaWorkspace() )
  {
    ...
    {
      if( numberOfArguments_ == 1 )
        throw EExpressionError( "GMEAN: argument workspace is empty" );
      else
        throw EExpressionError( "GMEAN: argument 1 workspace is empty" );
    }
  ...
  if( numberOfArguments_ == 2 )
  {
    ...
    if( arg2->IsaNVariable() )
    {
      ...
      if( ndm2 != 1 )
        throw EExpressionError( "GMEAN: argument 2 must be a vector" );
    ...
    else if( arg2->IsaWorkspace() )
    {
      ...
      if( w->IsEmpty() )
        throw EExpressionError( "GMEAN: argument 2 workspace is empty" );
  ...
  if( numberOfArguments_ == 2 )
  {
    ...
    //
    if( size1 != size2 )
      throw EExpressionError( "GMEAN: arguments must have the same length" );
    for( int i=0; i<size1; ++i )
    {
      wsum += d1[i];
      if( d2[i] < 0.0 )
        throw EExpressionError( "GMEAN: cannot calculate geometric mean with negative data" );
      gmean += d1[i]*log(d2[i]);
    }
    if( wsum == 0.0 )
      throw EExpressionError( "GMEAN: sum of the weights = 0" );
  }
