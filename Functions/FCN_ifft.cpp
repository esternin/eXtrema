...
void FCN_ifft::ProcessArgument( bool firstArgument, int ndmEff, int &nLoop )
{
  ...
  {
    if( ndmEff != 2 )
      throw EExpressionError("IFFT: first argument must be a matrix");
...
void FCN_ifft::ArrayEval( Workspace *ws )
{
  ...
  else
  {
    ...
    if( w->IsEmpty() )
      throw EExpressionError( "IFFT: first argument workspace is empty" );
    a.assign( w->GetData().begin(), w->GetData().end() );
    nrows = w->GetDimMag(0);
    ncols = w->GetDimMag(1);
  }
  if( ncols != 2 )
    throw EExpressionError( "IFFT: first argument matrix must have exactly 2 columns" );
  if( nrows <= 2 )
    throw EExpressionError( "IFFT: first argument matrix must have more than 2 rows" );
  ExprCodes *arg2 = NULL;
  ...
  else
  {
    if( arg2->IsaWorkspace() )
      ...
      throw EExpressionError("IFFT: second argument must be character");
    s.ToUpper();
    if( s != "AMP&PHASE" && s != "COS&SIN" )
      throw EExpressionError("IFFT: second argument must be either AMP&PHASE or COS&SIN");
  }
