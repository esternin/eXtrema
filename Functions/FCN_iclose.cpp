...
FCN_iclose FCN_iclose::fcn_iclose_;
...
void FCN_iclose::ProcessArgument( bool firstArgument, int ndmEff, int &nLoop )
{
  ...
  {
    if( ndmEff != 1 )
      throw EExpressionError("first argument of ICLOSE function must be a vector");
  ...
  {
    if( ndmEff != 0 )
      throw EExpressionError("second argument of ICLOSE function must be a scalar");
...
void FCN_iclose::ArrayEval( Workspace *ws )
{
  ...
  else    // argument 1 must be a workspace
  {
    ...
    if( w1->IsEmpty() )
      throw EExpressionError( "ICLOSE: first argument workspace is empty" );
