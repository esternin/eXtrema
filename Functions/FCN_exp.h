...
class FUNCTIONS_LINKAGE_ FCN_exp : public ScalarFunction
{
public:
  static FCN_exp *Definition()
  { return &fcn_exp_; }
  
  void ScalarEval( int j, std::vector<double> &rStack ) const
  {
    if( rStack[j] > log(std::numeric_limits<double>::max()) )
      throw EExpressionError("max value for exp exceeded");
    rStack[j] = exp( rStack[j] );
  }

private:
  FCN_exp() : ScalarFunction( "EXP" )
  {}
