...
void FCN_fold::ProcessArgument( bool firstArgument, int ndmEff, int &nLoop )
{
  ...
  {
    if( ndmEff!=1 )
      throw EExpressionError("first argument of FOLD function must be a vector");
  ...
  {
    if( ndmEff != 0 )
      throw EExpressionError("second argument of FOLD function must be a scalar");
...
void FCN_fold::ArrayEval( Workspace *ws )
{
  ...
  else    // argument 1 must be a workspace
  {
    ...
    if( w1->IsEmpty() )
      throw EExpressionError( "FOLD: first argument workspace is empty" );
  ...
  else if( arg2->IsaWorkspace() )
  {
    if( arg2->GetWorkspacePtr()->IsEmpty() )
      throw EExpressionError( "FOLD: second argument workspace is empty" );
    rows = arg2->GetWorkspacePtr()->GetValue();
  }
  else
    rows = arg2->GetConstantValue();
  if( rows == 0.0 )
    throw EExpressionError("number of rows specified in FOLD function cannot be zero");
  if( rows < 0.0 )
    throw EExpressionError("number of rows specified in FOLD function cannot be < 0");
  std::size_t nrows = static_cast<std::size_t>(rows);
  std::size_t ncols = size/nrows;
  if( nrows*ncols != size )
    throw EExpressionError("invalid FOLD function row size");
  std::size_t k3 = 0;
