...
class FUNCTIONS_LINKAGE_ FCN_tand : public ScalarFunction
{
public:
  static FCN_tand *Definition()
  { return &fcn_tand_; }
  
  void ScalarEval( int j, std::vector<double> &rStack ) const
  { rStack[j] = tan( rStack[j]*M_PI/180. ); }

private:
  FCN_tand() : ScalarFunction( "TAND" )
  {}
  
  static FCN_tand fcn_tand_;
};
