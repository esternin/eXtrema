...
class FUNCTIONS_LINKAGE_ FCN_log : public ScalarFunction
{
public:
  static FCN_log *Definition()
  { return &fcn_log_; }
  
  void ScalarEval( int j, std::vector<double> &rStack ) const
  {
    if( rStack[j] <= 0.0 )throw EExpressionError("LOG: argument <= 0");
    rStack[j] = log( rStack[j] );
  }

private:
  FCN_log() : ScalarFunction( "LOG" )
  {}
