...
FCN_stdev FCN_stdev::fcn_stdev_;
...
void FCN_stdev::ArrayEval( Workspace *ws )
{
  ...
  if( arg1->IsaNVariable() )
  {
    ...
    if( ndm1 != 1 )
    {
      if( numberOfArguments_ == 1 )
        throw EExpressionError( "STDEV: argument must be a vector" );
      else
        throw EExpressionError( "STDEV: argument 1 must be a vector" );
    }
  ...
  else if( arg1->IsaWorkspace() )
  {
    ...
    {
      if( numberOfArguments_ == 1 )
        throw EExpressionError( "STDEV: argument workspace is empty" );
      else
        throw EExpressionError( "STDEV: argument 1 workspace is empty" );
    }
  ...
  if( numberOfArguments_ == 2 )
  {
    ...
    if( arg2->IsaNVariable() )
    {
      ...
      if( ndm2 != 1 )
        throw EExpressionError( "STDEV: argument 2 must be a vector" );
    ...
    else if( arg2->IsaWorkspace() )
    {
      ...
      if( w->IsEmpty() )
        throw EExpressionError( "STDEV: argument 2 workspace is empty" );
  ...
  if( numberOfArguments_ == 2 )
  {
    ...
    //
    if( size1 != size2 )
      throw EExpressionError( "STDEV: arguments must have the same length" );
    ...
    if( wsum == 0.0 )
      throw EExpressionError( "STDEV: sum of the weights = 0" );
