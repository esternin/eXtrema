...
FCN_deconvolute FCN_deconvolute::fcn_deconvolute_;
...
void FCN_deconvolute::ProcessArgument( bool firstArgument, int ndmEff, int &nLoop )
{
  ...
  {
    if( ndmEff!=1 )
      throw EExpressionError("DECONVOLUTE: first argument must be a vector");
  ...
  {
    if( ndmEff != 1 )
      throw EExpressionError("DECONVOLUTE: second argument must be a vector");
...
void FCN_deconvolute::ArrayEval( Workspace *ws )
{
  ...
  else    // argument 1 must be a workspace
  {
    ...
    if( w->IsEmpty() )
      throw EExpressionError( "DECONVOLUTE: first argument workspace is empty" );
  ...
  if( sizeB/2*2 != sizeB )
    throw EExpressionError("DECONVOLUTE: length second argument is not even");
  if( sizeX != sizeB )
    throw EExpressionError("DECONVOLUTE: vector argument lengths must be equal");
  //
