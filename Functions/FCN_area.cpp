...
FCN_area FCN_area::fcn_area_;

void FCN_area::ProcessArgument( bool firstArgument, int ndmEff, int &nLoop )
{
  if( firstArgument )
  {
    if( ndmEff != 1 )throw EExpressionError("AREA: first argument must be a vector");
  ...
  else
  {
    ...
    if( numberOfArguments_==2 && ndmEff!=1 )
      throw EExpressionError("AREA: second argument must be a vector");
  }
}
...
void FCN_area::ArrayEval( Workspace *ws )
{
  ...
  else
  {
    ...
    if( w1->IsEmpty() )throw EExpressionError( "AREA: first argument workspace is empty" );
  ...
  else
  {
    ...
    if( w2->IsEmpty() )throw EExpressionError( "AREA: second argument workspace is empty" );
    d2.assign( w2->GetData().begin(), w2->GetData().end() );
  }
  std::size_t size2 = d2.size();
  //
  if( size2 != size1 )throw EExpressionError("AREA: vectors have different lengths");
  if( size1 < 2 )throw EExpressionError("AREA: vector length < 2");
  //
