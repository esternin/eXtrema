...
void FCN_max::ArrayEval( Workspace *ws )
{
  ...
  else if( arg1->IsaWorkspace() )
  {
    ...
    if( w1->IsEmpty() )
      throw EExpressionError( "MAX: argument 1 workspace is empty" );
  ...
  else
  {
    ...
    else if( arg2->IsaWorkspace() )
    {
      ...
      if( w2->IsEmpty() )
        throw EExpressionError( "MAX: argument 2 workspace is empty" );
    ...
    {
      if( ndm1 != 0 )
        throw EExpressionError( "MAX: all function arguments must be the same type" );
    }
    else if( ndm2 == 1 )
    {
      if( ndm1 != 1 )
    ...
    if( num1 != num2 )
      throw EExpressionError( "MAX: all function arguments must be the same size" );
    if( num1 == 0 )
    {
    ...
    while( narg < numberOfArguments_ )
    {
      ...
      else if( arg2->IsaWorkspace() )
      {
        Workspace *w2 = arg2->GetWorkspacePtr();
        if( w2->IsEmpty() )
          throw EExpressionError( "MAX: argument workspace is empty" );
        ndm2 = w2->GetNumberOfDimensions();
        if( ndm2 == 0 )
          val2 = w2->GetValue();
        else
      ...
      if( num1 == 0 )
      {
        ws->SetValue( max(ws->GetValue(),val2) );
      }
      else
      {
        for( int i=0; i<num1; ++i )ws->SetData( i, max(ws->GetData(i),d2[i]) );
      }
      ++narg;
    }
  }
