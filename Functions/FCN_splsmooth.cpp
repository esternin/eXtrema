...
FCN_splsmooth FCN_splsmooth::fcn_splsmooth_;
...
void FCN_splsmooth::ProcessArgument( bool firstArgument, int ndmEff, int &nLoop )
{
  ...
  {
    if( ndmEff != 1 )
      throw EExpressionError("SPLSMOOTH: first argument must be a vector");
  ...
  else
  {
    ...
    switch (++numberOfArguments_)
    {
      case 2:
        if( ndmEff != 1 )
          throw EExpressionError("SPLSMOOTH: second argument must be a vector");
        break;
      case 3:
        if( ndmEff != 0 )
          throw EExpressionError("SPLSMOOTH: third argument must be a scalar");
        break;
      case 4:
        if( ndmEff != 1 )
          throw EExpressionError("SPLSMOOTH: fourth argument must be a vector");
        break;
    }
  }
}
...
void FCN_splsmooth::ArrayEval( Workspace *ws )
{
  ...
  else
  {
    ...
    if( w1->IsEmpty() )
      throw EExpressionError( "SPLSMOOTH: first argument workspace is empty" );
  ...
  else
  {
    ...
    if( w2->IsEmpty() )
      throw EExpressionError( "SPLSMOOTH: second argument workspace is empty" );
    d2.assign( w2->GetData().begin(), w2->GetData().end() );
  }
  std::size_t size2 = d2.size();
  //
  if( size2 != size1 )
    throw EExpressionError("SPLSMOOTH: vectors have different lengths");
  if( size1 < 2 )
    throw EExpressionError("SPLSMOOTH: independent vector length < 2");
  //
  double d3;
  ...
  else if( arg3->IsaWorkspace() )
  {
    if( arg3->GetWorkspacePtr()->IsEmpty() )
      throw EExpressionError( "SPLSMOOTH: third argument workspace is empty" );
  ...
  if( npts < 2 )throw EExpressionError( "SPLSMOOTH: number of output points must be > 1" );
  //
  ...
  if( numberOfArguments_ == 4 ) // arg4 is the weights
  {
    ...
    else
    {
      ...
      if( w4->IsEmpty() )
        throw EExpressionError( "SPLSMOOTH: fourth argument workspace is empty" );
