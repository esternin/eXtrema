...
FCN_gaussj FCN_gaussj::fcn_gaussj_;
...
void FCN_gaussj::ProcessArgument( bool firstArgument, int ndmEff, int &nLoop )
{
  ...
  {
    if( ndmEff != 2 )
      throw EExpressionError("GAUSSJ: first argument must be a matrix");
  ...
  else
  {
    ...
    {
      if( ndmEff != 1 )
        throw EExpressionError("GAUSSJ: second argument must be a vector");
    }
  }
}
...
void FCN_gaussj::ArrayEval( Workspace *ws )
{
  ...
  else
  {
    ...
    if( w1->IsEmpty() )throw EExpressionError( "GAUSSJ: first argument workspace is empty" );
  ...
  if( size1a[0] != size1a[1] )
    throw EExpressionError("GAUSSJ: first argument must be a square matrix");
  int size1 = size1a[0];
  //
  ...
  else
  {
    ...
    if( w2->IsEmpty() )throw EExpressionError( "GAUSSJ: second argument workspace is empty" );
    d2.assign( w2->GetData().begin(), w2->GetData().end() );
  }
  int size2 = d2.size();
  //
  if( size2 != size1 )
    throw EExpressionError("GAUSSJ: row dimension of matrix first argument must be same as the length of the vector argument");
  if( size1 < 2 )
    throw EExpressionError("GAUSSJ: vector length < 2");
  //
  try
  {
