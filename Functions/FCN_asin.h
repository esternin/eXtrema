...
class FUNCTIONS_LINKAGE_ FCN_asin : public ScalarFunction
{
public:
  static FCN_asin *Definition()
  { return &fcn_asin_; }
  
  void ScalarEval( int j, std::vector<double> &rStack ) const
  {
    if( rStack[j]<-1.0 || rStack[j]>1.0 )
      throw EExpressionError("ASIN: argument must be >= -1 and <= 1");
    rStack[j] = asin( rStack[j] );
  }

private:
  FCN_asin() : ScalarFunction( "ASIN" )
  {}
