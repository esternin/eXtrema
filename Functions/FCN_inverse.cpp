...
FCN_inverse FCN_inverse::fcn_inverse_;
...
void FCN_inverse::ProcessArgument( bool firstArgument, int ndmEff, int &nLoop )
{
  ...
  {
    if( ndmEff != 2 )
      throw EExpressionError("INVERSE: argument must be a matrix");
...
void FCN_inverse::ArrayEval( Workspace *ws )
{
  ...
  else
  {
    ...
    if( w1->IsEmpty() )
      throw EExpressionError( "INVERSE: first argument workspace is empty" );
  ...
  if( size1a[0] != size1a[1] )
    throw EExpressionError("INVERSE: first argument must be a square matrix");
  int size1 = size1a[0];
  //
  if( size1 < 2 )
    throw EExpressionError("INVERSE: matrix dimension < 2");
  //
