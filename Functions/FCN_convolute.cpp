...
void FCN_convolute::ProcessArgument( bool firstArgument, int ndmEff, int &nLoop )
{
  ...
  {
    if( ndmEff!=1 )
      throw EExpressionError("CONVOLUTE: first argument must be a vector");
  ...
  {
    if( ndmEff != 1 )
      throw EExpressionError("CONVOLUTE: second argument must be a vector");
...
void FCN_convolute::ArrayEval( Workspace *ws )
{
  ...
  else    // argument 1 must be a workspace
  {
    ...
    if( w->IsEmpty() )
      throw EExpressionError( "CONVOLUTE: first argument workspace is empty" );
  ...
  if( sizeB/2*2 == sizeB )
  {
    even = true;
    if( sizeX != sizeB )
      throw EExpressionError("CONVOLUTE: vector argument lengths must be equal");
  }
  double sumB = 0.0;
