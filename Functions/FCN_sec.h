...
class FUNCTIONS_LINKAGE_ FCN_sec : public ScalarFunction
{
public:
  static FCN_sec *Definition()
  { return &fcn_sec_; }
  
  void ScalarEval( int j, std::vector<double> &rStack ) const
  {
    double c = cos(rStack[j]);
    if( c <= 0.0 )throw EExpressionError("SEC: cos(argument) = 0");
    rStack[j] = 1.0/c;
  }

private:
  FCN_sec() : ScalarFunction( "SEC" )
  {}
  
  static FCN_sec fcn_sec_;
};
