...
#
# Check for the wxGTK library. (See http://www.wxwidgets.org)
# The output variables WX_CXXFLAGSS and WX_LIBS will contain the
# compiler flags and libraries necessary to use wxGTK.
#
# The user must use --with-wxGTK=DIR which will cause the
# macro to look for wx-config in DIR, e.g.,
#   with-wxGTK=/usr/local/wxGTK-2.6.3/build-release
#
# test -n   test if length of string $WX_CONFIG is non-zero
# test -d   test if $with_wx exists and is a directory
#
AC_DEFUN([AX_CHECK_WX], [
  AC_ARG_WITH([wxGTK],AC_HELP_STRING([--with-wxGTK(=DIR)],[use the wxGTK wxWidgets package (in DIR)]),[],[])
  if test "$with_wxGTK" = "no"; then
    AC_MSG_ERROR([fatal error: wxGTK is required to build extrema])
  elif test "$with_wxGTK" = ""; then
    AC_PATH_PROG(WX_CONFIG,wx-config,[],[$PATH])
...
AC_DEFUN([AX_SETCXXFLAGS],[
  EX_CXXFLAGS='-I${top_srcdir}/src -I${top_srcdir}/src/Commands -I${top_srcdir}/src/Expression -I${top_srcdir}/src/Functions -I${top_srcdir}/src/Graphics -I${top_srcdir}/src/Operators -I${top_srcdir}/src/Parsing -I${top_srcdir}/src/Utilities -I${top_srcdir}/src/Variables -I${top_srcdir}/src/wxForms -I/usr/include/libxml2 -DEX_DATADIR="\"$(datadir)\"" -DEX_BINDIR="\"$(bindir)\""'
