...
FCN_iequal *FCN_iequal::fcn_iequal_ = 0;
...
void FCN_iequal::ProcessArgument( bool firstArgument, int ndmEff, int &nLoop )
{
  ...
  {
    if( ndmEff != 1 )
      throw EExpressionError( wxT("first argument of IEQUAL function must be a vector") );
  ...
  {
    if( ndmEff != 0 )
      throw EExpressionError( wxT("second argument of IEQUAL function must be a scalar") );
...
void FCN_iequal::ArrayEval( Workspace *ws )
{
  ...
  else    // argument 1 must be a workspace
  {
    ...
    if( w1->IsEmpty() )
      throw EExpressionError( wxT("IEQUAL: first argument workspace is empty") );
  ...
  {
    if( fabs(d1[i]-value) <= std::numeric_limits<double>::epsilon() )
    {
