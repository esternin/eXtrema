...
void FCN_min::ArrayEval( Workspace *ws )
{
  ...
  else if( arg1->IsaWorkspace() )
  {
    ...
    if( w1->IsEmpty() )
      throw EExpressionError( wxT("MIN: first argument workspace is empty") );
  ...
  else
  {
    ...
    else if( arg2->IsaWorkspace() )
    {
      ...
      if( w2->IsEmpty() )
        throw EExpressionError( wxT("MIN: second argument workspace is empty") );
    ...
    {
      if( ndm1 != 0 )
        throw EExpressionError( wxT("MIN: all function arguments must be the same type") );
    }
    else if( ndm2 == 1 )
    {
      if( ndm1 != 1 )
    ...
    if( num1 != num2 )
      throw EExpressionError( wxT("MIN: all function arguments must be the same size") );
    if( num1 == 0 )
    {
      ws->SetValue( std::min(val1,val2) );
      ws->SetNumberOfDimensions(0);
      ws->SetDimMag( 0, 0 );
    }
    else
    {
      for( int i=0; i<num1; ++i )ws->SetData( i, std::min(d1[i],d2[i]) );
    ...
    while( narg < numberOfArguments_ )
    {
      ...
      else if( arg2->IsaWorkspace() )
      {
        Workspace *w2 = arg2->GetWorkspacePtr();
        if( w2->IsEmpty() )
          throw EExpressionError( wxT("MIN: argument workspace is empty") );
        ndm2 = w2->GetNumberOfDimensions();
        if( ndm2 == 0 )
          val2 = w2->GetValue();
        else
      ...
      if( num1 != num2 )
        throw EExpressionError( wxT("MIN: function arguments must be the same size") );
      if( num1 == 0 )
        ws->SetValue( std::min(ws->GetValue(),val2) );
      else
        for( int i=0; i<num1; ++i )ws->SetData( i, std::min(ws->GetData(i),d2[i]) );
      ++narg;
    }
  }
}
