...
FCN_adev *FCN_adev::fcn_adev_ = 0;

void FCN_adev::ProcessArgument( bool firstArgument, int ndmEff, int &nLoop )
{
  if( firstArgument )
  {
...
void FCN_adev::ArrayEval( Workspace *ws )
{
  ...
  if( arg1->IsaNVariable() )
  {
    ...
    if( ndm1 != 1 )
    {
      if( numberOfArguments_ == 1 )
        throw EExpressionError( wxT("ADEV: argument must be a vector") );
      else
        throw EExpressionError( wxT("ADEV: argument 1 must be a vector") );
    }
  ...
  else if( arg1->IsaWorkspace() )
  {
    ...
    {
      if( numberOfArguments_ == 1 )
        throw EExpressionError( wxT("ADEV: argument workspace is empty") );
      else
        throw EExpressionError( wxT("ADEV: argument 1 workspace is empty") );
    }
  ...
  if( numberOfArguments_ == 2 )
  {
    ...
    if( arg2->IsaNVariable() )
    {
      ...
      if( ndm2 != 1 )
        throw EExpressionError( wxT("ADEV: argument 2 must be a vector") );
    ...
    else if( arg2->IsaWorkspace() )
    {
      ...
      if( w->IsEmpty() )
        throw EExpressionError( wxT("ADEV: argument 2 workspace is empty") );
  ...
  double wsum=0.0, xsum=0.0, adev=0.0;
  if( numberOfArguments_ == 2 )
  {
    ...
    //
    if( size1 != size2 )
      throw EExpressionError( wxT("ADEV: arguments must have the same length") );
    ...
    if( wsum == 0.0 )
      throw EExpressionError( wxT("ADEV: sum of the weights = 0") );
    double xmean = xsum/wsum;
    for( int i=0; i<size1; ++i )adev += d1[i]*fabs(d2[i]-xmean);
  }
  else // the argument is the data vector
  {
    ...
    double xmean = xsum/wsum;
    for( int i=0; i<size1; ++i )adev += fabs(d1[i]-xmean);
  }
  ws->SetValue( adev/wsum );
