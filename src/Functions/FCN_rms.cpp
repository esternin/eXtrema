...
FCN_rms *FCN_rms::fcn_rms_ = 0;

void FCN_rms::ProcessArgument( bool firstArgument, int ndmEff, int &nLoop )
{
  if( firstArgument )
  {
...
void FCN_rms::ArrayEval( Workspace *ws )
{
  ...
  if( arg1->IsaNVariable() )
  {
    ...
    if( ndm1 != 1 )
    {
      if( numberOfArguments_ == 1 )
        throw EExpressionError( wxT("RMS: argument must be a vector") );
      else
        throw EExpressionError( wxT("RMS: argument 1 must be a vector") );
    }
  ...
  else if( arg1->IsaWorkspace() )
  {
    ...
    {
      if( numberOfArguments_ == 1 )
        throw EExpressionError( wxT("RMS: argument workspace is empty") );
      else
        throw EExpressionError( wxT("RMS: argument 1 workspace is empty") );
    }
  ...
  if( numberOfArguments_ == 2 )
  {
    ...
    if( arg2->IsaNVariable() )
    {
      ...
      if( ndm2 != 1 )
        throw EExpressionError( wxT("RMS: argument 2 must be a vector") );
    ...
    else if( arg2->IsaWorkspace() )
    {
      ...
      if( w->IsEmpty() )
        throw EExpressionError( wxT("RMS: argument 2 workspace is empty") );
  ...
  if( numberOfArguments_ == 2 )
  {
    ...
    //
    if( size1 != size2 )
      throw EExpressionError( wxT("RMS: arguments must have the same length") );
    ...
    if( wsum == 0.0 )
      throw EExpressionError( wxT("RMS: sum of the weights = 0") );
  }
  else // the argument is the data vector
  {
    wsum = static_cast<double>(size1);
    for( int i=0; i<size1; ++i )rms += d1[i]*d1[i];
  }
  if( rms/wsum < 0.0 )
    throw EExpressionError( wxT("RMS: rms/(sum of weights) < 0") );
  ws->SetValue( sqrt(rms/wsum) );
