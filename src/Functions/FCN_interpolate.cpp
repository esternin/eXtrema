...
FCN_interpolate *FCN_interpolate::fcn_interpolate_ = 0;
...
void FCN_interpolate::ProcessArgument( bool firstArgument, int ndmEff, int &nLoop )
{
  ...
  {
    if( ndmEff != 1 )
      throw EExpressionError( wxT("INTERPOLATE: first argument must be a vector") );
  ...
  else
  {
    ...
    {
      if( ndmEff != 1 )
        throw EExpressionError( wxT("INTERPOLATE: second argument must be a vector") );
    }
    else if( numberOfArguments_ == 3 )
    {
      if( ndmEff != 1 )
        throw EExpressionError( wxT("INTERPOLATE: third argument must be a vector") );
    }
  }
}
...
void FCN_interpolate::ArrayEval( Workspace *ws )
{
  ...
  else
  {
    ...
    if( w1->IsEmpty() )throw EExpressionError( wxT("INTERPOLATE: first argument workspace is empty") );
  ...
  else
  {
    ...
    if( w2->IsEmpty() )
      throw EExpressionError( wxT("INTERPOLATE: second argument workspace is empty") );
    d2.assign( w2->GetData().begin(), w2->GetData().end() );
  }
  std::size_t size2 = d2.size();
  //
  if( size2 != size1 )
    throw EExpressionError( wxT("INTERPOLATE: vectors have different lengths") );
  if( size1 < 2 )
    throw EExpressionError( wxT("INTERPOLATE: independent vector length < 2") );
  //
  ...
  else
  {
    ...
    if( w3->IsEmpty() )
      throw EExpressionError( wxT("INTERPOLATE: third argument workspace is empty") );
  ...
  else
  {
    if( arg4->IsaWorkspace() )
      ...
      throw EExpressionError( wxT("INTERPOLATE: fourth argument must be character") );
    s4.UpperCase();
  }
  for( std::size_t i=1; i<size1; ++i )
  {
    if( fabs(d1[i]-d1[i-1]) < std::numeric_limits<double>::epsilon() )
      throw EExpressionError(
        wxT("INTERPOLATE: independent vector must be strictly monotonically increasing") );
  }
  if( s4 == wxT("LINEAR") )
  {
    for( std::size_t i=1; i<size3; ++i )
    {
      if( fabs(d3[i]-d3[i-1]) < std::numeric_limits<double>::epsilon() )
        throw EExpressionError(
          wxT("INTERPOLATE: output independent vector must be strictly monotonically increasing") );
    }
  }
  else
  {
    if( s4 == wxT("FC") && size1<3 )
      throw EExpressionError( wxT("INTERPOLATE: independent vector length < 3") );
    for( std::size_t i=1; i<size3; ++i )
    {
      if( d3[i] < d3[i-1] )
        throw EExpressionError(
          wxT("INTERPOLATE: output independent vector must be monotonically increasing") );
    }
  }
  ...
  else if( s4 == wxT("LINEAR") )
  {
    UsefulFunctions::LinearInterpolate( d1, d2, d3, yy );
  }
  else if( s4 == wxT("LAGRANGE") )
  {
    UsefulFunctions::LagrangeInterpolate( d1, d2, d3, yy );
  }
  else throw EExpressionError( wxT("INTERPOLATE: invalid fourth parameter") );
  //
