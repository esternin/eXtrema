      ...
      read_(read), message_(message), fileType_(fileType)
{
  ...
  wxButton *chooseFileButton = new wxButton( this, ID_chooseFile, wxT("Browse"), wxDefaultPosition, wxSize(8*GetCharWidth(),-1)); // 1.5*GetCharHeight()));
  chooseFileButton->SetToolTip( wxT("click to open a file browser") );
  sizer->Add( chooseFileButton, wxSizerFlags(0).Border(wxALL,5) );
  filenameComboBox_ = new wxComboBox( this, wxID_ANY, wxT(""), wxDefaultPosition, wxSize(80*GetCharWidth(), -1),//1.5*GetCharHeight()),
                                      0, 0, wxCB_READONLY );
