    ...
    : theX(x), theY(y), theZ(z)
{
  ...
  if( theNumberOfNodes < 3 )
    throw EGraphicsError( wxT("ThiessenTriangulation: number of nodes < 3") );
  //
...
void GRA_thiessenTriangulation::CreateMesh()
{
  ...
  size_t kmin;
L2:
  if( k == theNumberOfNodes )throw EGraphicsError( wxT("all nodes are collinear") );
  km1 = k++;
  xk = theX[k-1];
  yk = theY[k-1];
  dxk = xk-xl;
...
L1:
  ...
  wxT("Gradients: method failed to converge within given number of iterations") );
  dgmax = 0.;
  indl = 0;
  for( size_t k=1; k<=theNumberOfNodes; ++k )
  {
    ...
    dgmax = std::max(dgmax,std::max(fabs(dzx),fabs(dzy)));
    //
  ...
  /*
    ...
    double dzy = (a11*r2 - a12*r1)/(a11*a22 - a12*a12);
    double dzx = (r1 - a12*dzy)/a11;
    dgmax = std::max(std::max(dgmax,fabs(dzx)),fabs(dzy));
    //
    // update the partials at node k
    //
    xPartials[k] = zxk + dzx;
...
void GRA_thiessenTriangulation::AddNode( size_t k )
{
  ...
  if( i1 == 0 )
    throw EGraphicsError( wxT("all nodes are collinear") );
...
void GRA_thiessenTriangulation::AddBoundaryNode( size_t k, size_t right, size_t left )
{
  ...
  size_t nF = std::max(n1,n2);
  //
  // insert k as a neighbor of max(right,left)
  //
  Shift( nF, nL, 2, iadj );
  iadj[nF] = k;
  size_t imin = std::max(right,left);
...
void GRA_thiessenTriangulation::Shift( size_t nF, size_t nL, int k,
                                      ...
                                      size_t &i1, size_t &i2, size_t &i3 )
{
  ...
  //
  size_t n0 = std::max(nst,static_cast<size_t>(1));
...
void GRA_thiessenTriangulation::InterpolateSub( double xp, double yp, double &pz, size_t &ist )
{
  ...
  */
  ...
  if( i1 == 0 )throw EGraphicsError( wxT("InterpolateSub: nodes are collinear") );
  ist = i1;
  if( i3 == 0 )goto L3;
  //
                                        ...
                                        double zy1, double zy2, double zy3 )
{
  ...
  phi[2] = r[0]*r[1];
  //
  double rmin = std::min(std::min(r[0],r[1]),r[2]);
  size_t ip1, ip2, ip3;
  if( rmin != r[0] )goto L3;
  ip1 = 1;
  ip2 = 2;
