...
class VARIABLES_LINKAGE_ NumericData
{
private:
...
public:
  ...
  void SetData( std::size_t i, std::size_t j, double d )
  {
    std::size_t m = i+j*dimMag_[0];
    SetData( m, d );
  }

  void SetData( std::size_t i, std::size_t j, std::size_t k, double d )
  {
    std::size_t m = i+j*dimMag_[0]+k*dimMag_[0]*dimMag_[1];
    SetData( m, d );
  }
