...
void TextVariable::GetVariable( ExString const &name, bool flag, ExString &data )
{
  ...
  //
  if( name.empty() )throw EVariableError("variable name is blank");
  //
  if( !flag )
  {
    try
  ...
  else if( expr.IsNumeric() )
  {
    ...
    throw EVariableError( name+" is not character type" );
  }
}
...
std::ostream &operator<<( std::ostream &out, TextVariable const &tv )
{
  ...
  switch ( tv.data_.GetNumberOfDimensions() )
  {
    case 0: // scalar
    {
      out << "<textscalar name=\"" << tv.name_ << "\">\n"
          << "<string><![CDATA[" << tv.data_.GetScalarValue() << "]]></string>\n";
      closer = "</textscalar>\n";
      break;
    }
    case 1: // vector
    {
      ...
      out << "<textvector name=\"" << tv.name_ << "\" size=\"" << size << "\">\n";
      for( std::size_t i=0; i<size; ++i )
        out << "<string><![CDATA[" << const_cast<TextVariable&>(tv).data_.GetData(i)
            << "]]></string>\n";
      closer = "</textvector>\n";
      break;
    }
  ...
  std::deque<ExString>::const_reverse_iterator hEnd = tv.history_.rend();
  for( std::deque<ExString>::const_reverse_iterator i=tv.history_.rbegin(); i!=hEnd; ++i )
    out << "<string><![CDATA[" << *i << "]]></string>\n";
  return out << "</history>\n" << "<origin><![CDATA[" << tv.GetOrigin()
             << "]]></origin>\n" << closer;
}

ExString TextVariable::GetString( NumericData const *nda, NumericData const *ndb )
{
  ExString aString, finalString;
  if( nda )      // there is an array index
  {
    if( data_.GetNumberOfDimensions() == 0 )throw EVariableError(
      ExString("text scalar variable ")+name_+ExString(" cannot have an array index") );
    if( nda->GetNumberOfDimensions() > 0 )throw EVariableError(
      ExString("text array variable ")+name_+ExString(" array index must be scalar") );
    //
    int i = static_cast<int>( nda->GetScalarValue() );
    if( i > static_cast<int>( data_.GetData().size() ) )
      throw EVariableError( ExString("array index > maximum for ")+name_ );
    if( i <= 0 )
      throw EVariableError( ExString("array index <= 0 for ")+name_ );
  ...
  if( ndb )    // there are character indices
  {
    if( ndb->GetNumberOfDimensions() == 0 ) // scalar character index
    {
      ...
      if( jval <= 0 )
        throw EVariableError( ExString("character index <= 0 for ")+name_ );
      if( jval > static_cast<int>(aString.length()) )
        throw EVariableError( ExString("character index > maximum for ")+name_ );
    ...
    else if( ndb->GetNumberOfDimensions() == 1 )
    {
      ...
      throw EVariableError("character index can only be scalar or vector");
  }
...
void TextVariable::GetIndices( ExString &name, int &numberOfIndices,
                               std::vector<double> *indexVecs, bool *indexIsScalar )
{
  std::size_t dum = name.find( "//" );
  if( dum != name.npos() )
  {
  ...
  ExString indices( name.substr(simpleLen,indexLen) );
  //
  ...
  for( int i=0; i<indexLen; ++i )
  {
    ...
    if( indices[i]==')' || indices[i]=='}' || indices[i]==']' )
    {
      if( bracketCntr-- == 1 )
      {
        ExString idx = ExString("[") + indices.substr(start,i-start) + ExString("]");
        ...
        {                          // could be array or character index
          ...
          if( ndim == 0 )
          {
            if( static_cast<int>(nd.GetScalarValue()) < 0 )
              throw EVariableError( ExString("string variable index < 0: ")+idx );
          ...
          else if( ndim == 1 )
          {
            ...
            throw EVariableError( ExString("string variable index must be scalar or vector: ")+idx );
        }
        ...
        {                               // must be character index
          ...
          {
            if( static_cast<int>(nd.GetScalarValue()) < 0 )
              throw EVariableError( ExString("string variable character index < 0: ")+idx );
          ...
          throw EVariableError( ExString("string variable has too many indices: ")+idx );
      }
    }
  }
  if( bracketCntr != 0 )
    throw EVariableError( ExString("unmatched brackets in index: ")+name );
  name = simpleName;
}

bool TextVariable::DeletePartial( ExString &name, ExString const &inputLine )
{
  int numberOfIndices;
  ...
  if( numberOfIndices == 1 ) // one set of indices were used
  {
    ...
    {
      ExString s( data_.GetScalarValue() );
      int sLen = static_cast<int>(s.size());
      if( indexIsScalar[0] )  // destroy one character
      {
        int charIndex = static_cast<int>(indexVecs[0][0]);
        if( charIndex > sLen )
          throw EVariableError( ExString("character index > maximum (")+name+")" );
        if( charIndex < 1 )
          throw EVariableError( ExString("character index < 1 (")+name+")" );
        s.erase( charIndex-1 );
      }
    ...
    {
      std::vector<ExString> data( data_.GetData() );
      ...
      if( indexIsScalar[0] )  // destroy one string
      {
        ...
        if( arrayIndex > finalIndex )
          throw EVariableError( ExString("text array index > maximum (")+name+")" );
        if( arrayIndex < 1 )
          throw EVariableError( ExString("text array index < 1 (")+name+")" );
        data.erase( data.begin()+arrayIndex-1 );
      }
      else  // destroy multiple strings from array text variable
      {
        std::vector<ExString> tmp;
        int nValues = static_cast<int>(indexVecs[0].size());
  ...
  else if( numberOfIndices == 2 )
  {
    if( data_.GetNumberOfDimensions() == 0 )  // scalar text variable
    {
      ExGlobals::WarningMessage(
        ExString("array index not allowed on text scalar variables (")+name+")" );
      return false;
    }
    std::vector<ExString> data( data_.GetData() );
...
TextVariable *TextVariable::PutVariable( ExString name, ExString const &result,
                                         ExString const &inputLine, bool addToHistory )
{
  ...
  if( numberOfIndices == 2 )
  {
    ...
    if( indexIsScalar[0] )
      arrayIndex = indexVecs[0][0];
    else
      throw EVariableError(ExString("Error in ")+inputLine+ExString(": array index must be scalar"));
    if( indexIsScalar[1] )
    {
      characterIndex = indexVecs[1][0];
  ...
  if( arrayIndexUsed )    // output variable had an array index so it will be a text vector
  {
    ...
    {
      for( int i=tv->data_.GetData().size(); i<arrayIndex-1; ++i )tv->data_.AddString(ExString(" "));
    ...
    else                                // output variable does not exist
    {
      ...
      for( int i=0; i<arrayIndex-1; ++i )tv->data_.AddString( ExString(" ") );
      if( characterIndexUsed )          // there are character indices
      {
        ExString s( characterIndex-1, ' ' );
        if( scalarCharacterIndex )
          s.replace( characterIndex-1, 1, 1, result.c_str()[0] );
        else
        {
  ...
  else                    // output variable had no array indices
  {
    ...
    {                                   // so it must be a scalar
      if( characterIndexUsed )          // there are character indices
      {
        ExString s( tv->data_.GetScalarValue() );
...
TextVariable *TextVariable::PutVariable( ExString name,
                                         std::vector<ExString> const &results,
                                         ExString const &inputLine, bool addToHistory )
{
  // name       = name of the text variable without indices
  ...
  int maxCharacterIndex = numeric_limits<int>::min();
  //
  if( numberOfIndices == 2 )
  {
    arrayIndexUsed = true;
  ...
  {                       // so it will be a text vector
    if( results.size()!=1 && results.size()!=arrayIndex.size() )
      throw EVariableError( "array index length not equal to results length" );
    //
    if( preExisting )                   // output variable already exists
    {                                   // so it must be a vector
      std::size_t tvLength = tv->data_.GetData().size();
      std::size_t size = static_cast<std::size_t>(arrayIndex[0]-1);
      for( std::size_t i=tvLength; i<size; ++i )tv->data_.AddString( ExString(" ") );
      ...
      for( std::size_t i=0; i<size; ++i )
      {
        ...
        if( characterIndexUsed )
        {
          if( result.size() != characterIndex.size() )
           throw EVariableError( "character index length not equal to result length" );
          //
        ...
        if( i < arrayIndex.size()-1 )
        {
          ...
          {
            if( k-1 >= tvLength )tv->data_.AddString( ExString(" ") );
          }
        }
      }
    }
    else // output variable does not exist
    {
      tv = new TextVariable( name );
      tv->data_.SetNumberOfDimensions( 1 );
      for( int i=0; i<static_cast<int>(arrayIndex[0])-1; ++i )tv->data_.AddString( ExString(" ") );
  ...
  else // output variable had no array indices
  {
    ...
    {                                   // so it must be a scalar
      ...
      {
        if( results.size() != 1 )
         throw EVariableError( "no array indices given but results is an array" );
