...
{
  if( !v )throw EVariableError("attempt to add a null numeric variable");
  ExString name( v->GetName() );
  if( NVariableTable::Contains( name ) )
   throw EVariableError( ExString("The variable ")+name+ExString(" is already in the table") );
...
std::ostream &operator<<( std::ostream &out, NVariableTable const &table )
{
  if( table.vMap_.size() > 0 )
  {
    ...
    for( NVariableTable::NVariableMap::const_iterator i=table.vMap_.begin(); i!=end; ++i )out << *((*i).second);
