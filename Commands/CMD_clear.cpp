...
CMD_clear CMD_clear::cmd_clear_;

CMD_clear::CMD_clear() : Command( "CLEAR" )
{
  AddQualifier( "REPLOT", true );
  AddQualifier( "HISTORY", false );
}
...
void CMD_clear::Execute( ParseLine const *p )
{
  ...
  if( p->GetNumberOfTokens() == 1 ) // no parameters entered
  {
    ...
    if( qualifiers["REPLOT"] )ExGlobals::ClearWindows();
    return;
  }
  ...
  {
    throw ECommandError( "CLEAR", e.what() );
  }
  int n = static_cast<int>( d );
  if( n < 0 )throw ECommandError( "CLEAR", "window number < 0" );
  if( n > static_cast<int>(ExGlobals::GetNumberOfWindows()) )
   throw ECommandError( "CLEAR", "window number > largest defined window number" );
  GRA_window *gw = ExGlobals::GetGraphWindow( n );
  gw->Erase();
  gw->DisplayBackground( ExGlobals::GetScreenOutput() );
  if( qualifiers["REPLOT"] )gw->Clear();
