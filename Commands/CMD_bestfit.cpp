...
CMD_bestfit CMD_bestfit::cmd_bestfit_;

CMD_bestfit::CMD_bestfit() : Command( "BESTFIT" )
{
  AddQualifier( "WEIGHTS", false );
  AddQualifier( "CYCLES", false );
}
...
void CMD_bestfit::Execute( ParseLine const *p )
{
  ...
  if( qualifiers["WEIGHTS"] )
  {
    ...
    if( p->GetNumberOfTokens() < counter+1 )
      throw ECommandError( "BESTFIT", "expecting weight vector" );
    try
    {
      NumericVariable::GetVector( p->GetString(counter), "weight vector", weights );
    }
    catch( EVariableError &e )
    {
      throw ECommandError( "BESTFIT", e.what() );
    }
    nwt = weights.size();
    AddToStackLine( p->GetString(counter++) );
  }
  int ncyc = 10;
  if( qualifiers["CYCLES"] )
  {
    // BESTFIT\CYCLES           n pmin pmax penalty error parm pout
    // BESTFIT\CYCLES\WEIGHTS w n pmin pmax penalty error parm pout
    //
    ...
    try
    {
      NumericVariable::GetScalar( p->GetString(counter), "number of cycles", v );
    }
    catch( EVariableError &e )
    {
      throw ECommandError( "BESTFIT", e.what() );
    ...
    if( ncyc <= 0 )
      throw ECommandError( "BESTFIT", "number of cycles must be > 0" );
    AddToStackLine( p->GetString(counter++) );
  }
  std::vector<double> pmin;
  try
  {
    NumericVariable::GetVector( p->GetString(counter), "parameter minima", pmin );
  }
  catch( EVariableError &e )
  {
    throw ECommandError( "BESTFIT", e.what() );
  ...
  try
  {
    NumericVariable::GetVector( p->GetString(counter), "parameter maxima", pmax );
  }
  catch( EVariableError &e )
  {
    throw ECommandError( "BESTFIT", e.what() );
  ...
  try
  {
    NumericVariable::GetVector( p->GetString(counter), "parameter penalties", penalties );
  }
  catch( EVariableError &e )
  {
    throw ECommandError( "BESTFIT", e.what() );
  ...
  try
  {
    NumericVariable::GetVector( p->GetString(counter), "error vector", errors );
  }
  catch( EVariableError &e )
  {
    throw ECommandError( "BESTFIT", e.what() );
  ...
  if( nwt != nerror )throw ECommandError( "BESTFIT",
    "weight vector length must equal the error vector length" );
  std::vector<double> pcm;
  std::size_t nr, np;
  try
  {
    NumericVariable::GetMatrix( p->GetString(counter), "parameter characteristic matrix",
                                pcm, nr, np );
  }
  catch( EVariableError &e )
  {
  ...
  AddToStackLine( p->GetString(counter++) );
  if( nerror != static_cast<int>(nr) )throw ECommandError( "BESTFIT",
    "error vector length must equal the number of rows of the matrix" );
  if( npmin != static_cast<int>(np) )throw ECommandError( "BESTFIT",
    "parameter minimum vector length must equal the number of columns of the matrix" );
  if( npmax != static_cast<int>(np) )throw ECommandError( "BESTFIT",
    "parameter maximum vector length must equal the number of columns of the matrix" );
  if( npenalties != static_cast<int>(np) )throw ECommandError( "BESTFIT",
    "parameter penalty vector length must equal the number of columns of the matrix" );
  if( p->GetNumberOfTokens()<counter+1 || !p->IsString(counter) )
      throw ECommandError( "BESTFIT", "expecting output parameter vector name" );
  //
  ...
  ExString poutName( p->GetString(counter) );
  try
  {
                           ...
                           std::vector<double> &p )
{
  ...
  for( ;; )
  {
    ...
    if( !Simq(ainv,dt) )
      throw ECommandError( "BESTFIT", "the matrix is singular" );
    ...
    if( kill == 0 ) // no violation of maximum constraints
    {
      ...
      if( cyclesUsed > ncyc )
      {
        ExGlobals::WriteOutput( ExString("limit on cycles reached, with parameter ")+
                                ExString(ip[kill]+1)+ExString(" still outside") );
        FitDataInfo( cyclesUsed, np, nr, p, weights, errors, pcm );
        return;
      }
      p[ikill-1] = pmin[ikill-1];
      ExGlobals::WriteOutput( ExString("parameter ")+ExString(ikill)+
                              ExString(" < min ")+ExString(pmin[ikill-1])+
                              ExString(" so set to min") );
      //
    ...
    else
    {
      ...
      ExGlobals::WriteOutput( ExString("parameter ")+ExString(ikill-1)+
                              ExString(" > max ")+ExString(pmax[ikill-1])+
                              ExString(" so set to max") );
      //
      // Modify  contribution to vector D due to parameter ip[kill]
                               ...
                               std::vector<double> const &pcm )
{
  ...
  //
  ExGlobals::WriteOutput( ExString("number of cycles used: ")+ExString(cyclesUsed) );
  double sumpar = 0.0;
  for( int i=0; i<np; ++i )
  {
    sumpar += p[i]*p[i];
    ExGlobals::WriteOutput( ExString("variable ")+ExString(i+1)+
                            ExString(" change = ")+ExString(p[i]) );
  }
  double resi = 0.0;
  double resf = 0.0;
  for( int j=0; j<nr; ++j )
  ...
  ExGlobals::WriteOutput( ExString("rms parameter change = ")+ExString(sparn) );
  ExGlobals::WriteOutput( ExString("    initial residual = ")+ExString(resi) );
  ExGlobals::WriteOutput( ExString("      final residual = ")+ExString(resf) );
}
