...
OPR_union OPR_union::opr_union_;
                                ...
                                int &nDim, int &nLoop )
{
  ...
  {
    if( ndmEff != 1 )
      throw EExpressionError( "union first operand must be a vector" );
    code->SetIndx(0,0);
  }
  else
  {
    if( ndmEff != 1 )
      throw EExpressionError( "union second operand must be a vector" );
    nDim = 1;
    nLoop = 1;
    code->SetIndx(0,1);
    //
                             ...
                             int *nptEff, Workspace *ws )
{
  ...
  else
  {
    ...
    if( c->IsaNVariable() )
      ...
      throw EExpressionError( "union second operand problem" );
    if( ndim != 1 )
      throw EExpressionError( "union second operand must be a vector: 2" );
...
void OPR_union::ArrayEval( Workspace *ws )
{
  ...
  else
  {
    ...
    if( w1->IsEmpty() )
      throw EExpressionError( "union operator: first operand workspace is empty" );
  ...
  else
  {
    ...
    if( w2->IsEmpty() )
      throw EExpressionError( "union operator: second operand workspace is empty" );
