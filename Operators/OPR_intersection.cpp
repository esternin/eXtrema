...
OPR_intersection OPR_intersection::opr_intersection_;
                                       ...
                                       int &nDim, int &nLoop )
{
  ...
  {
    if( ndmEff != 1 )
      throw EExpressionError( "intersection first operand must be a vector" );
    code->SetIndx(0,0);
  }
  else
  {
    if( ndmEff != 1 )
      throw EExpressionError( "intersection second operand must be a vector" );
    nDim = 1;
    nLoop = 1;
    code->SetIndx(0,1);
    //
                                    ...
                                    int *nptEff, Workspace *ws )
{
  ...
  else
  {
    ...
    if( c->IsaNVariable() )
      ...
      throw EExpressionError( "intersection second operand problem" );
    if( ndim != 1 )
      throw EExpressionError( "intersection second operand must be a vector: 2" );
...
void OPR_intersection::ArrayEval( Workspace *ws )
{
  ...
  else
  {
    ...
    if( w1->IsEmpty() )
      throw EExpressionError( "intersection operator: first operand workspace is empty" );
  ...
  else
  {
    ...
    if( w2->IsEmpty() )
      throw EExpressionError( "intersection operator: second operand workspace is empty" );
  ...
  if( cntr == 0 )throw EExpressionError( "intersection is empty" );
  ws->SetDimMag( 0, cntr );
}
