...
class OPERATORS_LINKAGE_ OperatorDefinition
{
protected:
  enum Type { SCALAR, ARRAY, CHARACTER };

  OperatorDefinition( ExString const &name, int up, int bp, Type type )
      : name_(name), unaryPriority_(up), binaryPriority_(bp), type_(type)
  { OperatorTable::GetTable()->AddEntry( this ); }
