...
OPR_transpose OPR_transpose::opr_transpose_;
                                    ...
                                    int &nDim, int &nLoop )
{
  ...
  {
    if( ndmEff != 2 )
      throw EExpressionError( "transpose operand must be a matrix" );
    code->SetIndx( 0, 0 );
    code->SetIndx( 1, 1 );
  }
}
...
void OPR_transpose::ArrayEval( Workspace *ws )
{
  ...
  else
  {
    ...
    if( w1->IsEmpty() )
      throw EExpressionError( "transpose operator: operand workspace is empty" );
