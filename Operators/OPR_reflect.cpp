...
OPR_reflect OPR_reflect::opr_reflect_;
                                  ...
                                  int &nDim, int &nLoop )
{
  ...
  {
    if( ndmEff != 2 )
      throw EExpressionError( "reflect operand must be a matrix" );
    code->SetIndx( 0, 0 );
    code->SetIndx( 1, 1 );
  }
}
...
void OPR_reflect::ArrayEval( Workspace *ws )
{
  ...
  else
  {
    ...
    if( w1->IsEmpty() )
      throw EExpressionError( "reflect operator: operand workspace is empty" );
