...
void GRA_threeDPlot::Draw( GRA_outputType *graphicsOutput )
{
  ...
  graphicsOutput->SetLineType( static_cast<GRA_intCharacteristic*>(curveC->Get("CURVELINETYPE"))->Get() );
  graphicsOutput->SetLineWidth( static_cast<GRA_intCharacteristic*>(curveC->Get("CURVELINEWIDTH"))->Get() );

  graphicsOutput->SetColor( static_cast<GRA_colorCharacteristic*>(curveC->Get("CURVECOLOR"))->Get() );
  DrawCurve( graphicsOutput );
  graphicsOutput->SetColor( colorSave );
  DrawSymbols( graphicsOutput );
  DrawAxes( textC, graphicsOutput );
...
void GRA_threeDPlot::ScaleXY( double const x, double const y, double const z, double &scx, double &scy )
{
  double xminW, yminW, xmaxW, ymaxW;
  ExGlobals::GetWorldLimits( xminW, yminW, xmaxW, ymaxW );
  scx = ((xmaxW-xminW)*ds_/screenHalfSize_*x/(eye2object_+z)+(xminW+xmaxW)*screenHalfSize_)/(2*screenHalfSize_);
  scy = ((ymaxW-yminW)*ds_/screenHalfSize_*y/(eye2object_+z)+(yminW+ymaxW)*screenHalfSize_)/(2*screenHalfSize_);
}

void GRA_threeDPlot::DrawCurve( GRA_outputType *graphicsOutput )
{
  double xs, ys;
  ...
  for( std::size_t i=1; i<npt; ++i )
  {
    ...
    plotSymbols_[i]->GetConnectToPrevious() ? graphicsOutput->ContinueLine( xs, ys ) :
                                              graphicsOutput->StartLine( xs, ys );
  }
}

void GRA_threeDPlot::DrawSymbols( GRA_outputType *graphicsOutput )
{
  std::size_t npt = x_.size();
  for( std::size_t i=0; i<npt; ++i )
  {
...
void GRA_threeDPlot::DrawAxes( GRA_setOfCharacteristics *textC, GRA_outputType *graphicsOutput )
{
  ...
  for( int i=0; i<nxyz; ++i )
  {
    ...
    p2_[i] == 2 ? graphicsOutput->ContinueLine( xs, ys ) : graphicsOutput->StartLine( xs, ys );
  }
  double xs1, ys1;
  ScaleXY( xn_[0], yn_[0], zn_[0], xs1, ys1 );
  static_cast<GRA_intCharacteristic*>(textC->Get("ALIGNMENT"))->Set( 6 );
  static_cast<GRA_distanceCharacteristic*>(textC->Get("XLOCATION"))->SetAsWorld( xs1 );
  static_cast<GRA_distanceCharacteristic*>(textC->Get("YLOCATION"))->SetAsWorld( ys1 );
  GRA_drawableText *dt = new GRA_drawableText( ExString(zmax_) );
  dt->Parse();
  ...
  static_cast<GRA_intCharacteristic*>(textC->Get("ALIGNMENT"))->Set( 6 );
  static_cast<GRA_distanceCharacteristic*>(textC->Get("XLOCATION"))->SetAsWorld( xs2 );
  static_cast<GRA_distanceCharacteristic*>(textC->Get("YLOCATION"))->SetAsWorld( ys2 );
  dt = new GRA_drawableText( ExString(zmin_) );
  dt->Parse();
  graphicsOutput->Draw( dt );
  ...
  double yc = (ys1+ys2)/2.;
  static_cast<GRA_intCharacteristic*>(textC->Get("ALIGNMENT"))->Set( 6 );
  static_cast<GRA_distanceCharacteristic*>(textC->Get("XLOCATION"))->SetAsWorld( xc );
  static_cast<GRA_distanceCharacteristic*>(textC->Get("YLOCATION"))->SetAsWorld( yc );
  dt = new GRA_drawableText( "Z" );
  dt->Parse();
  graphicsOutput->Draw( dt );
  ...
  static_cast<GRA_distanceCharacteristic*>(textC->Get("XLOCATION"))->SetAsWorld( xs1 );
  static_cast<GRA_distanceCharacteristic*>(textC->Get("YLOCATION"))->SetAsWorld( ys1 );
  dt = new GRA_drawableText( ExString(xmin_) );
  dt->Parse();
  graphicsOutput->Draw( dt );
  ...
  static_cast<GRA_distanceCharacteristic*>(textC->Get("XLOCATION"))->SetAsWorld( xs2 );
  static_cast<GRA_distanceCharacteristic*>(textC->Get("YLOCATION"))->SetAsWorld( ys2 );
  dt = new GRA_drawableText( ExString(xmax_) );
  dt->Parse();
  graphicsOutput->Draw( dt );
  ...
  static_cast<GRA_distanceCharacteristic*>(textC->Get("XLOCATION"))->SetAsWorld( xc );
  static_cast<GRA_distanceCharacteristic*>(textC->Get("YLOCATION"))->SetAsWorld( yc );
  dt = new GRA_drawableText( "X" );
  dt->Parse();
  graphicsOutput->Draw( dt );

  ScaleXY( xn_[4], yn_[4], zn_[4], xs1, ys1 );
  static_cast<GRA_intCharacteristic*>(textC->Get("ALIGNMENT"))->Set( 4 );
  static_cast<GRA_distanceCharacteristic*>(textC->Get("XLOCATION"))->SetAsWorld( xs1 );
  static_cast<GRA_distanceCharacteristic*>(textC->Get("YLOCATION"))->SetAsWorld( ys1 );
  dt = new GRA_drawableText( ExString(ymin_) );
  dt->Parse();
  graphicsOutput->Draw( dt );
  ...
  static_cast<GRA_distanceCharacteristic*>(textC->Get("XLOCATION"))->SetAsWorld( xs2 );
  static_cast<GRA_distanceCharacteristic*>(textC->Get("YLOCATION"))->SetAsWorld( ys2 );
  dt = new GRA_drawableText( ExString(ymax_) );
  dt->Parse();
  graphicsOutput->Draw( dt );
