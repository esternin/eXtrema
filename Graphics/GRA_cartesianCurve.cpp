...
{
  DeleteStuff();
  if( popup_ )CurvePopupForm->Disconnect();
}
...
void GRA_cartesianCurve::SetUp()
{
  ...
  GRA_intCharacteristic *psCharacteristic =
    static_cast<GRA_intCharacteristic*>(curveC->Get("PLOTSYMBOL"));
  ...
  GRA_sizeCharacteristic *pssizeCharacteristic =
    static_cast<GRA_sizeCharacteristic*>(curveC->Get("PLOTSYMBOLSIZE"));
  ...
  GRA_angleCharacteristic *psangleCharacteristic =
    static_cast<GRA_angleCharacteristic*>(curveC->Get("PLOTSYMBOLANGLE"));
  ...
  GRA_colorCharacteristic *pscolorCharacteristic =
    static_cast<GRA_colorCharacteristic*>(curveC->Get("PLOTSYMBOLCOLOR"));
  ...
  GRA_intCharacteristic *pslwCharacteristic =
    static_cast<GRA_intCharacteristic*>(curveC->Get("PLOTSYMBOLLINEWIDTH"));
  std::vector<int> lws;
  ...
  //
  for( std::size_t i=0; i<xSize; ++i )plotsymbols_.push_back(
    new GRA_plotSymbol(symbols[i],sizes[i],angles[i],colors[i],lws[i]) );
  //
  histogramType_ =
    static_cast<GRA_intCharacteristic*>(curveC->Get("HISTOGRAMTYPE"))->Get();
  lineType_ =
    static_cast<GRA_intCharacteristic*>(curveC->Get("CURVELINETYPE"))->Get();
  lineWidth_ =
    static_cast<GRA_intCharacteristic*>(curveC->Get("CURVELINEWIDTH"))->Get();
  color_ =
    static_cast<GRA_colorCharacteristic*>(curveC->Get("CURVECOLOR"))->Get();
  GRA_colorCharacteristic *areaFillColorCharacteristic =
    static_cast<GRA_colorCharacteristic*>(generalC->Get("AREAFILLCOLOR"));
  ...
  //
  xuaxis_ = static_cast<GRA_distanceCharacteristic*>(xAxisC->Get("UPPERAXIS"))->GetAsWorld();
  xlaxis_ = static_cast<GRA_distanceCharacteristic*>(xAxisC->Get("LOWERAXIS"))->GetAsWorld();
  yuaxis_ = static_cast<GRA_distanceCharacteristic*>(yAxisC->Get("UPPERAXIS"))->GetAsWorld();
  ylaxis_ = static_cast<GRA_distanceCharacteristic*>(yAxisC->Get("LOWERAXIS"))->GetAsWorld();
  xmin_ = static_cast<GRA_doubleCharacteristic*>(xAxisC->Get("MIN"))->Get();
  xmax_ = static_cast<GRA_doubleCharacteristic*>(xAxisC->Get("MAX"))->Get();
  ymin_ = static_cast<GRA_doubleCharacteristic*>(yAxisC->Get("MIN"))->Get();
  ymax_ = static_cast<GRA_doubleCharacteristic*>(yAxisC->Get("MAX"))->Get();
}
...
void GRA_cartesianCurve::Draw( GRA_outputType *graphicsOutput )
{
  ...
  switch ( histogramType_ )
  {
    case 1:
    case 3:
      DrawHistogramNoTails( graphicsOutput );
      break;
    case 2:
    case 4:
      DrawHistogramWithTails( graphicsOutput );
      break;
    default:
      DrawNonHistogram( graphicsOutput );
  }
  DrawErrorBars( graphicsOutput );
...
void GRA_cartesianCurve::DrawNonHistogram( GRA_outputType *graphicsOutput )
{
  ...
  if( areaFillColor_ )
  {
    ...
    std::auto_ptr<GRA_polygon> p( new GRA_polygon(xCurve_,yCurve_,color_,areaFillColor_,lineWidth_) );
    graphicsOutput->Draw( p.get() );
    if( test )
    {
  ...
  for( std::size_t i=0; i<size; ++i )
  {
    pen_[i]==2 ? graphicsOutput->ContinueLine( xCurve_[i], yCurve_[i] ) :
  ...
  for( std::size_t i=0; i<size; ++i )
  {
    if( xmin<=xCurve_[i] && xCurve_[i]<=xmax && ymin<=yCurve_[i] && yCurve_[i]<=ymax )
    {
      ...
      graphicsOutput->Draw( plotsymbols_[i] );
    }
  }
...
void GRA_cartesianCurve::DrawHistogramNoTails( GRA_outputType *graphicsOutput )
{
  if( !areaFillColors_.empty() )
  {
    areaFillColor_ = areaFillColors_[0];
  ...
  for( std::size_t i=1; i<size; ++i )graphicsOutput->PenDown( xCurve_[i], yCurve_[i] );
}

void GRA_cartesianCurve::DrawHistogramWithTails( GRA_outputType *graphicsOutput )
{
  std::size_t npt = xData_.size();
  if( npt > 1 )
  {
  ...
  {
    graphicsOutput->PenUp( xCurve_[0], yCurve_[0] );
    graphicsOutput->PenDown( xCurve_[1], yCurve_[1] );
  }
  else
  {
    for( std::size_t i=0; i<npt; ++i )
    {
      ...
      graphicsOutput->PenDown( xCurve_[j+1], yCurve_[j+1] );
      graphicsOutput->PenDown( xCurve_[j+2], yCurve_[j+2] );
      graphicsOutput->PenDown( xCurve_[j+3], yCurve_[j+3] );
      graphicsOutput->PenDown( xCurve_[j],   yCurve_[j] );
    }
  }
}
...
void GRA_cartesianCurve::DrawErrorBars( GRA_outputType *graphicsOutput )
{
