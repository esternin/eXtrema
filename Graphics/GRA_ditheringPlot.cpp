...
void GRA_ditheringPlot::Draw( GRA_outputType *graphicsOutput )
{
...
void GRA_ditheringPlot::DrawBitmap( GRA_outputType *outputType )
{
  double amin, amax;
  ...
  else
  {
    if( contourLevels_[0] <= amin )
      throw EGraphicsError("first contour level <= data minimum");
    if( contourLevels_[nLevels-2] >= amax )
      throw EGraphicsError("last contour level >= data maximum");
...
void GRA_ditheringPlot::FillBitmap( GRA_bitmap *bitmap )
{
  ...
  for( int i=0; i<h; ++i )
  {
    for( int j=0; j<w; ++j )
    {
      if( iicc[j+i*w] < 0 )bitmap->SetPixelColor( j, i, GRA_colorControl::GetColor("BLACK") );
    }
  }
  if( lines_ )  // draw the contour lines processed vertically
  {
    ...
    for( int j=0; j<w; ++j )
    {
      for( int i=0; i<h; ++i )
      {
        ...
        if( icc>0 && icc!=iccold )bitmap->SetPixelColor( j, i, GRA_colorControl::GetColor("BLACK") );
        iccold = icc;
      }
    }
    //
...
void GRA_ditheringPlot::DrawLegend( GRA_outputType *outputType )
{
  ...
  if( height > 0.0 )
  {
    for( std::size_t i=0; i<=nLevels; ++i )
    {
      ...
      char c[50];
      sprintf( c, "%9.2le", contourVec_[i] );
      double angle = 0.0;
      GRA_font *font = static_cast<GRA_fontCharacteristic*>(generalC->Get("FONT"))->Get();
      std::auto_ptr<GRA_drawableText> dt(
       new GRA_drawableText(ExString(c),height,angle,xhi+0.1,yp-height/2.,1,
  ...
  for( std::size_t icc=1; icc<=nLevels; ++icc )
  {
    ...
    for( int i=0; i<leg[icc-1]; ++i )
    {
      for( int j=0; j<w; ++j )
      {
        ...
        if( plot )bitmap->SetPixelColor( j, ny, GRA_colorControl::GetColor("BLACK") );
      }
      if( ++ny == h-1 )break;
    }
    for( int j=0; j<w; ++j )bitmap->SetPixelColor( j, ny, GRA_colorControl::GetColor("BLACK") );
  }
}
