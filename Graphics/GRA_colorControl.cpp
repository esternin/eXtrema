...
{
extern GRA_colorMap *current_ =0;
extern GRA_colorMap *userdefined_ =0;
extern GRA_colorMap *thermal_ =0;
extern GRA_colorMap *gray256_ =0;
extern GRA_colorMap *gray16_ =0;
extern GRA_colorMap *rainbow_ =0;
extern GRA_colorMap *redblue16_ =0;
extern GRA_colorMap *classic8_ =0;
extern GRA_colorMap *topo256_ =0;
extern GRA_colorMap *default_ =0;
...
void SetFile( ExString const &fileName )
{
  std::ifstream readIn;
  readIn.open( fileName.c_str(), ios_base::in );
  if( !readIn )
  {
  ...
  for( int record=0; ; ++record )
  {
    ...
    tmp->AddColor( new GRA_color(rs.ToInt(),gs.ToInt(),bs.ToInt()) );
  }
  delete userdefined_;
  userdefined_ = tmp;
...
{
  namedColors_.push_back( new GRA_color("WHITE",255,255,255) );
  namedColors_.push_back( new GRA_color("BLACK",0,0,0) );
  namedColors_.push_back( new GRA_color("RED",255,0,0) );
  namedColors_.push_back( new GRA_color("GREEN",0,255,0) );
  namedColors_.push_back( new GRA_color("BLUE",0,0,255) );
  namedColors_.push_back( new GRA_color("PURPLE",160,32,240) );
  namedColors_.push_back( new GRA_color("YELLOW",255,255,0) );
  namedColors_.push_back( new GRA_color("CYAN",0,255,255) );
  namedColors_.push_back( new GRA_color("BROWN",165,42,42) );
  namedColors_.push_back( new GRA_color("CORAL",255,127,80) );
  namedColors_.push_back( new GRA_color("SALMON",250,128,114) );
  namedColors_.push_back( new GRA_color("SIENNA",160,82,45) );
  namedColors_.push_back( new GRA_color("TAN",210,180,140) );
  namedColors_.push_back( new GRA_color("FUCHSIA",255,0,255) );
  namedColors_.push_back( new GRA_color("LIME",50,205,50) );
  namedColors_.push_back( new GRA_color("MAROON",128,0,0) );
  namedColors_.push_back( new GRA_color("NAVY",0,0,128) );
  namedColors_.push_back( new GRA_color("OLIVE",128,128,0) );
  namedColors_.push_back( new GRA_color("SILVER",192,192,192) );
  namedColors_.push_back( new GRA_color("TEAL",0,128,128) );
}

void SetColorMap( ExString const &inName )
{
  ExString name( inName.UpperCase() );
  if( name == ExString("THERMAL") )current_ = thermal_;
  else if( name == ExString("GRAY256") )current_ = gray256_;
  else if( name == ExString("GRAY16") )current_ = gray16_;
  else if( name == ExString("RAINBOW") )current_ = rainbow_;
  else if( name == ExString("REDBLUE16") )current_ = redblue16_;
  else if( name == ExString("CLASSIC8") )current_ = classic8_;
  else if( name == ExString("TOPO256") )current_ = topo256_;
  else if( name == ExString("DEFAULT") )current_ = default_;
  else if( name == ExString("USERDEFINED") && userdefined_ )current_ = userdefined_;
  else throw EGraphicsError( "unknown color map" );
}

int GetColorCode( ExString const &name )
{
  ...
  {
    if( name.UpperCase() == namedColors_[i]->GetName() )return -1*static_cast<int>(i);
  }
  ExString message("color ");
  message += name + ExString(" not found");
...
GRA_color *GetColor( ExString const &name )
{
  int size = static_cast<int>(namedColors_.size());
  for( int i=0; i<size; ++i )
  {
    if( name.UpperCase() == namedColors_[i]->GetName() )return namedColors_[i];
  }
  return reinterpret_cast<GRA_color*>(0);
}

void CreateThermalColorMap()
{
  thermal_ = new GRA_colorMap( "THERMAL" );
...
{
  rainbow_ = new GRA_colorMap( "RAINBOW" );
...
{
  redblue16_ = new GRA_colorMap( "REDBLUE16" );
...
{
  classic8_ = new GRA_colorMap( "CLASSIC8" );
...
{
  topo256_ = new GRA_colorMap( "TOPO256" );
...
{
  default_ = new GRA_colorMap( "DEFAULT" );
  for( int i=0; i<6; ++i )
  {
    for( int j=0; j<6; ++j )
    {
