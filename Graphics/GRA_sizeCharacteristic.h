...
{
public:
  GRA_sizeCharacteristic( char const *name, double value, bool percent, double min, double max )
      : GRA_characteristic(name,SIZE,false),
        min_(min), max_(max), value_(value), setAsPercent_(percent), default_(value)
  {}

  GRA_sizeCharacteristic( char const *name, std::vector<double> const &values, bool percent,
