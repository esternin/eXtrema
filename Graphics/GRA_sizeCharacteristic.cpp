...
{
  double result;
  setAsPercent_ ? result=value_ : result=value_/(max_-min_)*100.0;
...
std::vector<double> GRA_sizeCharacteristic::GetAsPercents() const
{
  ...
  {
    setAsPercent_ ? result=(*i) : result=(*i)/(max_-min_)*100.0;
...
{
  double result;
  setAsPercent_ ? result=value_*(max_-min_)/100.0 : result=value_;
...
std::vector<double> GRA_sizeCharacteristic::GetAsWorlds() const
{
  ...
  for( std::vector<double>::const_iterator i=values_.begin(); i!=end; ++i )
  {
    setAsPercent_ ? result=(*i)*(max_-min_)/100.0 : result=(*i);
