...
std::ostream &operator<<( std::ostream &out, GRA_doubleCharacteristic const &c )
{
  if( c.isAVector_ )
  {
    std::size_t size = c.values_.size();
    out << "<characteristic name=\"" << c.name_ << "\""
        << " type=\"DOUBLE\" shape=\"VECTOR\">\n"
        << "<data size=\"" << size << "\">";
  ...
  else
  {
    ...
    {
      out << "<characteristic name=\"" << c.name_ << "\""
          << " type=\"DOUBLE\" shape=\"SCALAR\" value=\"" << c.value_ << "\"/>\n";
    }
  }
  return out;
}

void GRA_doubleCharacteristic::CopyStuff( GRA_doubleCharacteristic const &rhs )
{
  value_ = rhs.value_;
