...
void GRA_diffusionPlot::Draw( GRA_outputType *graphicsOutput )
{
...
void GRA_diffusionPlot::DrawBitmap( GRA_outputType *graphicsOutput )
{
  //
  ...
  std::auto_ptr<GRA_bitmap> bitmap(
   new GRA_bitmap(xlo_,ylo_,xhi_,yhi_,GRA_colorControl::GetColor("WHITE"),graphicsOutput) );
  FillBitmap( bitmap.get() );
  graphicsOutput->Draw( bitmap.get() );
}

void GRA_diffusionPlot::FillBitmap( GRA_bitmap *bitmap )
{
  GRA_color *color = GRA_colorControl::GetColor("BLACK");
  //
