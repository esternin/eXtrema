...
bool GRA_axis::Inside( double xLoc, double yLoc )
{
  ...
  double axisAngle =
    static_cast<GRA_angleCharacteristic*>(characteristics_->Get("AXISANGLE"))->Get();
  ...
  double xup = xOrigin_ + (length_+fudge)*cosA + 2*fudge*sinA;
  double yup = yOrigin_ + 2*fudge*cosA + (length_+fudge)*sinA;
  return (xup-xLoc)*(xLoc-xlo)>0.0 && (yup-yLoc)*(yLoc-ylo)>0.0;
}

void GRA_axis::Make()
{
  if( static_cast<GRA_boolCharacteristic*>(characteristics_->Get("AXISON"))->Get() )
  {
    try
    {
      static_cast<GRA_doubleCharacteristic*>(characteristics_->Get("LOGBASE"))->Get()<=1.0 ?
...
void GRA_axis::Draw( GRA_outputType *graphicsOutput )
{
...
void GRA_axis::MakeLinearAxis()
{
  ...
  double const axisMax =
    static_cast<GRA_doubleCharacteristic*>(characteristics_->Get("MAX"))->Get();
  double const axisMin =
    static_cast<GRA_doubleCharacteristic*>(characteristics_->Get("MIN"))->Get();
  if( fabs(axisMax-axisMin) < std::numeric_limits<double>::epsilon() )return;
  //
  double const virtualMax =
    static_cast<GRA_doubleCharacteristic*>(characteristics_->Get("VIRTUALMAX"))->Get();
  double const virtualMin =
    static_cast<GRA_doubleCharacteristic*>(characteristics_->Get("VIRTUALMIN"))->Get();
  //
  int const powerAuto =
    static_cast<GRA_intCharacteristic*>(characteristics_->Get("POWERAUTO"))->Get();
  double base =
    static_cast<GRA_doubleCharacteristic*>(characteristics_->Get("LOGBASE"))->Get();
  ...
  //
  double numberAngle =
    static_cast<GRA_angleCharacteristic*>(characteristics_->Get("NUMBERSANGLE"))->Get();
  ...
  //
  bool tics =
    static_cast<GRA_boolCharacteristic*>(characteristics_->Get("TICSON"))->Get();
  bool ticsBothSides =
    static_cast<GRA_boolCharacteristic*>(characteristics_->Get("TICSBOTHSIDES"))->Get();
  int grid =
    static_cast<GRA_intCharacteristic*>(characteristics_->Get("GRID"))->Get();
  //
  double ticAngle =
    static_cast<GRA_angleCharacteristic*>(characteristics_->Get("TICANGLE"))->Get();
  ...
  //
  double imagTicAngle =
    static_cast<GRA_angleCharacteristic*>(characteristics_->Get("IMAGTICANGLE"))->Get();
  NormalizeAngle( imagTicAngle );
  double imagTicLength =
    static_cast<GRA_sizeCharacteristic*>(characteristics_->Get("IMAGTICLENGTH"))->GetAsWorld();
  ...
  int numberOfLargeIncrements =
    static_cast<GRA_intCharacteristic*>(characteristics_->Get("NLINCS"))->Get();
  numberOfLargeIncrements = std::max(1,numberOfLargeIncrements);
  double largeTicLength =
    static_cast<GRA_sizeCharacteristic*>(characteristics_->Get("LARGETICLENGTH"))->GetAsWorld();
  //
  ...
  int numberOfSmallIncrements =
    static_cast<GRA_intCharacteristic*>(characteristics_->Get("NSINCS"))->Get();
  numberOfSmallIncrements = std::max(1,numberOfSmallIncrements);
  double smallTicLength =
    static_cast<GRA_sizeCharacteristic*>(characteristics_->Get("SMALLTICLENGTH"))->GetAsWorld();
  //
  ...
  //
  bool numbers = static_cast<GRA_boolCharacteristic*>(characteristics_->Get("NUMBERSON"))->Get();
  //
  // width in characters of the axis number format
  // Maximum value is 20. If <= 0 then no axis numbers will be plotted
  //
  int numberOfDigits =
    static_cast<GRA_intCharacteristic*>(characteristics_->Get("NUMBEROFDIGITS"))->Get();
  ...
  int numberOfDecimals =
    static_cast<GRA_intCharacteristic*>(characteristics_->Get("NUMBEROFDECIMALS"))->Get();
  //
  // height of the axis number labels, if numberHeight <= 0, no numbers will be plotted
  //
  double numberHeight =
    static_cast<GRA_sizeCharacteristic*>(characteristics_->Get("NUMBERSHEIGHT"))->GetAsWorld();
  ...
  //
  int const lineWidth =
    static_cast<GRA_intCharacteristic*>(characteristics_->Get("LINEWIDTH"))->Get();
  int const lineType = 1;
  GRA_color *color =
    static_cast<GRA_colorCharacteristic*>(characteristics_->Get("AXISCOLOR"))->Get();
  //
  bool dropFirstNumber = static_cast<GRA_boolCharacteristic*>(characteristics_->Get("DROPFIRSTNUMBER"))->Get();
  bool dropLastNumber = static_cast<GRA_boolCharacteristic*>(characteristics_->Get("DROPLASTNUMBER"))->Get();
  double axisModulo = static_cast<GRA_doubleCharacteristic*>(characteristics_->Get("MOD"))->Get();
  double offSet = static_cast<GRA_doubleCharacteristic*>(characteristics_->Get("OFFSET"))->Get();
  double power = static_cast<GRA_doubleCharacteristic*>(characteristics_->Get("POWER"))->Get();
  bool leadingZeros = static_cast<GRA_boolCharacteristic*>(characteristics_->Get("LEADINGZEROS"))->Get();
  GRA_font *numbersFont = static_cast<GRA_fontCharacteristic*>(characteristics_->Get("NUMBERSFONT"))->Get();
  GRA_color *numbersColor = static_cast<GRA_colorCharacteristic*>(characteristics_->Get("NUMBERSCOLOR"))->Get();
  //
  // draw the axis line
  //
  ...
  while( virtualMin-i*xinc >= axisMin )
  {
    ...
    int modinc = fmod( i++, numberOfSmallIncrements );
    double ticlen;
    modinc==0 ? ticlen=largeTicLength : ticlen=smallTicLength;
    if( modinc == 0 )
    {
      if( grid != 0 )
      {
  ...
  for( int i=0; i<ntics; ++i )
  {
        ...
        (!dropFirstNumber || i!=0) && (!dropLastNumber || i!=ntics-1) )
    {
      ...
      //
      ExString number =
       DoubleToString( xdum2, static_cast<int>(power), numberOfDigits, numberOfDecimals );
      number.LeftJustify();
...
void GRA_axis::MakeLogAxis()
{
  ...
  if( fabs(axisMax-axisMin)<std::numeric_limits<double>::epsilon() )return;
  //
  double const base = static_cast<GRA_doubleCharacteristic*>(characteristics_->Get("LOGBASE"))->Get();
  ...
  //                           10    10   10   10
  //
  bool logStyle = static_cast<GRA_boolCharacteristic*>(characteristics_->Get("LOGSTYLE"))->Get();
  if( !logStyle && fabs(base-exp(1.0))<0.02 )logStyle = true;
  //
  // axisAngle = angle of the axis in degrees relative to the horizontal
  //
  double axisAngle = static_cast<GRA_angleCharacteristic*>(characteristics_->Get("AXISANGLE"))->Get();
  NormalizeAngle( axisAngle );
  double cosAxisAngle = cos(axisAngle*degToRad);
  double sinAxisAngle = sin(axisAngle*degToRad);
  if( fabs(cosAxisAngle) <= eps )cosAxisAngle = 0.0;
  if( fabs(sinAxisAngle) <= eps )sinAxisAngle = 0.0;
  //
  int const lineWidth = static_cast<GRA_intCharacteristic*>(characteristics_->Get("LINEWIDTH"))->Get();
  int const lineType = 1;
  GRA_color *color = static_cast<GRA_colorCharacteristic*>(characteristics_->Get("AXISCOLOR"))->Get();
  //
  double numberAngle = static_cast<GRA_angleCharacteristic*>(characteristics_->Get("NUMBERSANGLE"))->Get();
  NormalizeAngle( numberAngle );
  numberAngle -= axisAngle; // angle of the axis numbers in degrees, w.r.t. the horizontal
  //
  double ticAngle = static_cast<GRA_angleCharacteristic*>(characteristics_->Get("TICANGLE"))->Get();
  NormalizeAngle( ticAngle );
  ...
  double smallTicLength =
    static_cast<GRA_sizeCharacteristic*>(characteristics_->Get("SMALLTICLENGTH"))->GetAsWorld();
  //
  bool tics = static_cast<GRA_boolCharacteristic*>(characteristics_->Get("TICSON"))->Get();
  if( !tics )
  {
    largeTicLength = 0.0;
    smallTicLength = 0.0;
  }
  bool ticsBothSides = static_cast<GRA_boolCharacteristic*>(characteristics_->Get("TICSBOTHSIDES"))->Get();
  int grid = static_cast<GRA_intCharacteristic*>(characteristics_->Get("GRID"))->Get();
  double imagTicLength =
    static_cast<GRA_sizeCharacteristic*>(characteristics_->Get("IMAGTICLENGTH"))->GetAsWorld();
  //
  ...
  // number will be centered on its perimeter at the large tic mark locations
  //
  double imagTicAngle = static_cast<GRA_angleCharacteristic*>(characteristics_->Get("IMAGTICANGLE"))->Get();
  NormalizeAngle( imagTicAngle );
  ...
  //
  ExString label = fabs(base-exp(1.0))<0.02 ? ExString('e') : ExString(intBase);
  int alignment;
  if( fabs(axisAngle) <= 45.0 )
    alignment = imagTicAngle>180.0 ? 8 : 2; // 8=top centre, 2=bottom centre
  else
  ...
  if( powMin < axisMin )
  {
    ...
    for( int i=0; i<numberOfSmallTics; ++i )
    {
      ...
      {
        double stic = fractionOfGap[i]*gapLength - vminLoc; // distance from lAxis to tic mark
        ...
        if( numberSmallTics )
        {
          ...
          if( actualValue>9999999.0 || actualValue<0.0000001 ) // too many digits, use log style
          {
            ...
            sprintf( s, "%024u", i+2 );
            std::size_t idc1;
            for( idc1=0; idc1<strlen(s); ++idc1 )
            {
              if( s[idc1] != '0' )break;
            }
            for( std::size_t i=idc1; i<strlen(s); ++i )lab2 += ExString( s[i] );
            lab2 += ExString("*") + label + ExString("<^>") + ExString(powMin);
          }
          else
          {
            char s[25];
            if( actualValue >= 1 )sprintf( s, "%024u", static_cast<int>(actualValue+0.5) );
            else                  sprintf( s, "%024.7g", actualValue );
            std::size_t idc1;
            for( idc1=0; idc1<strlen(s); ++idc1 )
            {
              if( s[idc1] != '0' )break;
  ...
  for( int ipow=powMin; ipow<=powMax; ++ipow )
  {
    ...
    if( numbers )
    {
      ...
      else // draw decimal style numbers, e.g., 0.001, 0.01
      {
        ...
        {
          char s[25];
          sprintf( s, "%024.7f", xout );
          for( idc1=0; idc1<strlen(s); ++idc1 )
          {
            if( s[idc1] != '0' )break;
          }
          int idc2;
          for( idc2=strlen(s)-1; idc2>=0; --idc2 )
          {
            if( s[idc2] != '0' )break;
          }
          if( s[idc1] == '.' )--idc1;
          if( s[idc2] == '.' )--idc2;
          for( int i=idc1; i<=idc2; ++i )lab2 += ExString( s[i] );
        }
      }
...
void GRA_axis::MakeATic( double stic,
                         double cosAxisAngle, double sinAxisAngle,
                         double costic, double sintic,
                         double ticLength, double &xs, double &ys )
{
  xs = xOrigin_ + cosAxisAngle*stic;
  ys = yOrigin_ + sinAxisAngle*stic;
  //
...
void GRA_axis::Fnice()
{
  ...
  //
  double virtualMax = static_cast<GRA_doubleCharacteristic*>(characteristics_->Get("VIRTUALMAX"))->Get();
  double virtualMin = static_cast<GRA_doubleCharacteristic*>(characteristics_->Get("VIRTUALMIN"))->Get();
  double vmin = fabs(virtualMin);
  double vmax = fabs(virtualMax);
  int numberOfLargeIncrements = static_cast<GRA_intCharacteristic*>(characteristics_->Get("NLINCS"))->Get();
  double inc = fabs((virtualMax-virtualMin)/numberOfLargeIncrements);
  if( inc == 0.0 )inc = 1.0;
  //
  int numberOfDigits = static_cast<GRA_intCharacteristic*>(characteristics_->Get("NUMBEROFDIGITS"))->Get();
  numberOfDigits *= -1;
  int nwidor = numberOfDigits;      // original
  int numberOfDecimals = static_cast<GRA_intCharacteristic*>(characteristics_->Get("NUMBEROFDECIMALS"))->Get();
  ...
  /*
  double ipart;
  double dum = modf(inc/10.0,&ipart);
  //double dum = fmod(inc,10.0);
  ExString tmpstr("inc=");
  tmpstr += ExString(inc)+", "+ExString(dum)+", vmin: "+ExString(fmod(vmin,10.0))+
            ", vmax: "+ExString(fmod(vmax,10.0));
  ExGlobals::WriteOutput( tmpstr );
  */
  double const eps = 0.0001;
  ...
  /*
  tmpstr = "vmin=";
  tmpstr += ExString(vmin)+", vmax="+ExString(vmax)+", inc="+
            ExString(inc)+", ndec="+ExString(numberOfDecimals);
  ExGlobals::WriteOutput( tmpstr );
  */
  //
  ...
  //
  numberOfDigits = std::max(nwidmin,nwidmax);
  /*
  tmpstr = "ndec=";
  tmpstr += ExString(numberOfDecimals)+", ndecor="+ExString(ndecor)+", ndig="+
            ExString(numberOfDigits)+", nwidor="+ExString(nwidor);
  ExGlobals::WriteOutput( tmpstr );
  */
  ...
  static_cast<GRA_intCharacteristic*>(characteristics_->Get("NUMBEROFDIGITS"))->Set( numberOfDigits );
  static_cast<GRA_intCharacteristic*>(characteristics_->Get("NUMBEROFDECIMALS"))->Set( numberOfDecimals );
  static_cast<GRA_doubleCharacteristic*>(characteristics_->Get("POWER"))->Set( power );
}

ExString GRA_axis::DoubleToString( double r, int power, int numberOfDigits, int numberOfDecimals ) const
{
  // DoubleToString converts the double r into a string s, with format
  // f(numberOfDigits).(numberOfDecimals) such that: r = s*pow(10,power), and the string s is
  // right-justified in the field width of numberOfDigits characters.
  //
  // convert the scaled number r/pow(10,power) into s
  //
  // if the number of decimal places, numberOfDecimals < 0, then the decimal point
  // in the character representation of r will be suppressed, i.e., it
  // is assumed to be on the far right of the numberOfDigits characters of s.
  //
  if( numberOfDigits <= 0 )return ExString(" ");
  char digit[] = "0123456789";
  char minus='-', dec='.', blank=' ', star='*';
  //
  ...
  //
  int numberOfDecimals1 = std::max(numberOfDecimals,-1);
  int numberOfDecimals2 = std::max(numberOfDecimals, 0);
  int nint = numberOfDigits-numberOfDecimals1-1;
  if( r < 0.0 )--nint;
  if( nint<0 || nint+numberOfDecimals2<=0 )return ExString(numberOfDigits,star);
  //
  ...
  //
  double powdec = pow(10.0,numberOfDecimals2);
  double ra = fabs(r);
  //
  ...
  //
  ExString s( numberOfDigits, blank );
  double realn = powdec*ra + 0.5;
  for( int i=0; i<numberOfDigits; ++i )
  {
    if( i!=numberOfDecimals2 || numberOfDecimals1<0 )
    {
      int index = fmod(realn,10.0);
      if( index<0 || index>9 )index = 0;
      s[numberOfDigits-i-1] = digit[index];
      realn /= 10.0;
    }
    else
    {
      s[numberOfDigits-i-1] = dec;
    }
  }
  // if realn >= 1 then realn has overflowed the format, because there are
  // still non-zero leading digits which do not fit into the numberOfDigits positions of s
  //
  if( realn >= 1.0 )return ExString(numberOfDigits,star);
  //
  // determine the first non-zero character
  //
  std::size_t i = s.find_first_not_of( digit[0], 0 );
  if( i == s.npos() )i = numberOfDigits-1;
  ...
  if( r >= 0.0 )
  {
    ...
    //
    if( s[i] == '.' && i > 0 )s[i-1] = digit[0];
  }
  else     // r < 0, so put a minus sign
  {
    if( i == 0 )
    {
      s.insert( 0, numberOfDigits, star );
    }
    else
    {
      if( s[i] == '.' )  // first non-zero character is a '.'
      {
        if( i == 1 )
        {
          s[0] = minus;
  ...
  s.LeftJustify();  // strip off leading blanks
  return s;
}

std::ostream &operator<<( std::ostream &out, GRA_axis const &axis )
{
  out << "<axis xorigin=\"" << axis.xOrigin_ << "\" yorigin=\"" << axis.yOrigin_ << "\" length=\"" << "\">\n";
  ...
  out << "</ticcoordinates>\n<axisc>\n";
  out << *axis.characteristics_ << "</axisc>\n";
